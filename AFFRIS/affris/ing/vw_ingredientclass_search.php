...
$mode=SEARCH_SIMPLE;
$templatefile = "vw_ingredientclass_search.htm";

// for ajax query, used when page buffers new control
if(postvalue("mode")=="inlineLoadCtrl"){
	$mode = SEARCH_LOAD_CONTROL;
	$templatefile = "vw_ingredientclass_inline_search.htm";
}	
...
$params['xt'] = &$xt;
$params['shortTableName'] = 'vw_ingredientclass';
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if(GetLookupTable("Description4", $strTableName))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][GetLookupTable("Description4", $strTableName)] = GetTableURL(GetLookupTable("Description4", $strTableName));
	
	$pageObject->fillFieldToolTips("Description4");	
		
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("Description4");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "Description4";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("Description4_label","<label for=\"".GetInputElementId("Description4", $id)."\">","</label>");
	else 
		$xt->assign("Description4_label", true);
	
	$xt->assign("Description4_fieldblock", true);		
	$xt->assignbyref("Description4_editcontrol", $ctrlBlockArr['searchcontrol']);					
	$xt->assign("Description4_notbox", $ctrlBlockArr['notbox']);		
	// create second control, if need it		
	$xt->assignbyref("Description4_editcontrol1", $ctrlBlockArr['searchcontrol1']);		
	// create search type select
	$xt->assign("searchtype_Description4", $ctrlBlockArr['searchtype']);	
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("Description4");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl) 
	{				
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"Description4", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{	
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"Description4", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	...
	// search fields data
	$categoryFieldParams = $pageObject->searchClauseObj->getSearchCtrlParams("Description4");
	$categoryFieldVal = '';
	...
	$fieldParams = $pageObject->searchClauseObj->getSearchCtrlParams('IName');
	$fieldVal = '';
	...
	// search fields data
	$categoryFieldParams = $pageObject->searchClauseObj->getSearchCtrlParams("Description2");
	$categoryFieldVal = '';
	if (count($categoryFieldParams))
	{
		$categoryFieldVal = $categoryFieldParams[0]['value1'];
	...
	$fieldParams = $pageObject->searchClauseObj->getSearchCtrlParams('Description4');
	$fieldVal = '';
	if (count($fieldParams))
	{
		$fieldVal = $fieldParams[0]['value1'];
