...
if($inlineadd == ADD_INLINE)
	$templatefile = "vw_fullingredientproxanalysis_inline_add.htm";
else
	$templatefile = "vw_fullingredientproxanalysis_add.htm";
...
//	processing ETValue - start
	...
	if($inlineAddOption)
	{
		$value = postvalue("value_ETValue_".$id);
		$type=postvalue("type_ETValue_".$id);
		if (FieldSubmitted("ETValue_".$id))
		{
		...
		if(!($value===false))
		{
						...
						if(0 && "ETValue"=="pass" && $url_page=="admin_users_")
				$value=md5($value);
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if(GetTableData($strTableName,".edit",false) && $permis['edit'])
						$message .='&nbsp;<a href=\'vw_fullingredientproxanalysis_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if(GetTableData($strTableName,".view",false) && $permis['search'])
						$message .='&nbsp;<a href=\'vw_fullingredientproxanalysis_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: vw_fullingredientproxanalysis_".$pageObject->getPageType().".php");
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("ETValue"))
		$xt->assign("ETValue_fieldblock",true);
	else
		$xt->assign("ETValue_tabfieldblock",true);
...
//	ETValue - Number
	...
	$showValues["ETValue"] = $value;
	$showFields[] = "ETValue";
		$showRawValues["ETValue"] = substr($data["ETValue"],0,100);
	}	
//	IisDetail - 
	$display = false;
	if($inlineadd==ADD_MASTER)
