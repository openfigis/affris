...
//	check all locking records
		$rstmp=$this->TableObj->Query(AddFieldWrappers("table")."='".db_addslashes($strtable)."' and ".AddFieldWrappers("keys")."='".db_addslashes($skeys)."' and ".AddFieldWrappers("action")."=1",AddFieldWrappers("id")." asc");
...
//	check if locking was removed by admin or not
			$rstmp=$this->TableObj->Query(AddFieldWrappers("table")."='".db_addslashes($strtable)."' and ".AddFieldWrappers("keys")."='".db_addslashes($skeys)."' and ".AddFieldWrappers("sessionid")."<>'".session_id()."' and ".AddFieldWrappers("action")."=2",AddFieldWrappers("id")." asc");
	...
	function GetLockInfo($strtable,$keys,$links, $pageid)
	{
		...
		$rstmp=$this->TableObj->Query(AddFieldWrappers("table")."='".db_addslashes($strtable)."' and ".AddFieldWrappers("keys")."='".db_addslashes($skeys)."' and ".AddFieldWrappers("sessionid")."<>'".session_id()."' and ".AddFieldWrappers("action")."=1",AddFieldWrappers("id")." asc");
		if($data = db_fetch_array($rstmp))
		{
			$sdate=now();
			...
			{
				$str.="&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
				$str.="<a class=admin_links href='#' onclick=\"UnlockAdmin('".$page."','".htmlspecialchars(jsreplace($strtable))."','".htmlspecialchars(jsreplace($skeys))."','".$data["sessionid"]."','no',".$pageid.");return false;\">"."Unlock record"."</a>";
				$str.="&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a class=admin_links href='#' onclick=\"UnlockAdmin('".$page."','".htmlspecialchars(jsreplace($strtable))."','".htmlspecialchars(jsreplace($skeys))."','".$data["sessionid"]."','yes',".$pageid.");return false;\">"."Edit record"."</a>";
			}
			return $str;
		}
		else
...
//	delete all other locking records 
		$rstmp=CustomQuery("delete from ".AddTableWrappers($this->lockTableName)." where ".AddFieldWrappers("table")."='".db_addslashes($strtable)."' and ".AddFieldWrappers("keys")."='".db_addslashes($skeys)."' and ".AddFieldWrappers("action")."=1 and ".AddFieldWrappers("sessionid")."<>'".session_id()."' ");
