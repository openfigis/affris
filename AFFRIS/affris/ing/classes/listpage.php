...
class ListPage extends RunnerPage 
{
	...
	function prepareForResizeColumns()
	{
		...
		$this->jsSettings['tableSettings'][$this->tName]['numRows'] = ($this->numRowsFromSQL ? true : false);
	}
	...
	function deleteRecords() 
	{
		...
		foreach($this->selectedRecs as $keys) 
		{
			...
			if($this->lockingObj)
			{
				...
				$lockSQL="select * from ".AddTableWrappers("")." where ".AddFieldWrappers("keys")."='".db_addslashes($lockWhere)."' and ".AddFieldWrappers("table")."='".db_addslashes($this->origTName)."' and ".AddFieldWrappers("action")."=1";
		...
		{
			if($this->eventExists("AfterMassDelete"))
	...
	function commonAssign() 
	{
		...
		$this->xt->enable_section("usermessage");
		$this->xt->enable_section("message_block");
		...
		if($this->isCreateMenu() || $this->isAdminTable()) 
			$this->xt->assign("quickjump_attrs", 
							  "onfocus =\"window.selectcurrent = this.selectedIndex;\" 
	...
	function addAssignForGrid()
	{
		...
		if($this->isDispGrid()) 
		{
			...
			if(!$this->numRowsFromSQL){
				...
				$this->xt->assign("gridFooter_attrs", 'id="gridFooterTr'.$this->id.'" style="display: none;"');
			}
	...
	function checkboxColumnAttrs()
	{
		...
		$this->xt->assign("checkboxheader_attrs", "id=\"chooseAll_".$this->id."\"");
	}
	/**
	...
	function fillGridShowInfo(&$rowInfoArr) 
	{
		...
		{		
			...
			$row = array();
			$row["rowattrs"] = "id=\"addarea".$this->id."\" style=\"display: none;\"";
			if($this->isVerLayout)
...
//	add container for inline add controls
			...
			for($i = 0; $i < count($this->allDetailsTablesArr); $i ++) 
			{
				...
				$record[$dShortTable."_dtablelink_attrs"] = " href=\"".$dShortTable."_list.php?\" id=\"master_".$dShortTable."_add".$this->id."\"";
			...
			for($i = 0; $i < count($this->listFields); $i ++) 
			{
				...
				$record[GoodFieldName($this->listFields[$i]['fName'])."_style"] = $this->setAttrAlign($i);
			}
			if($this->colsOnPage > 1)
	...
	{		
		...
		while($data && ($this->recNo <= $this->pageSize || $this->pageSize == -1)) 
		{
			...
			{				
				...
				if($col==1 && $this->recNo==1 && $this->isResizeColumns)
					$this->jsSettings['tableSettings'][$this->tName]['recIdStart'] = $this->recId;
...
//				$this->controlsMap['gridRows'][$gridRowInd]['canEdit'] = !isset($_SESSION["_".$this->sessionPrefix."_OwnerID"]) || @$_SESSION["_".$this->sessionPrefix."_OwnerID"] == $data[GetTableOwnerID($this->tName)];
				...
				if($this->lockingObj)
				{
					if(!count($this->lockDelRec) && isset($_SESSION[$this->sessionPrefix."_lockDelRec"]))
					{
				...
				{					
					...
					$record[$this->listFields[$i]['styleFieldName']] = $this->setAttrAlign($i);
				}		
		...
		if($this->lockingObj && count($lockRecIds))
	...
	function setAttrAlign($i)
	{
		...
		elseif($this->listFields[$i]['viewFormat']==FORMAT_AUDIO)
			return 'align="center" style="vertical-align:top;"';
		else
			return 'align="left"';	
	}
			...
			if($dpreview) 
			{
				...
				{					
					$record[$dShortTable."_dtablelink_attrs"] = "
						...
						"onclick = \"window['dpInline".$this->id."'].showDPInline('".htmlspecialchars(jsreplace($dDataSourceTable))."',".$this->recId.",this); return false;\"" : "").
	...
	function & createListPage($table,$options)
	{
		...
		$params["isDynamicPerm"] = GetGlobalData("isDynamicPerm",false);
		$params['isAddWebRep'] = GetGlobalData("isAddWebRep",false);
