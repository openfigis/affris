	...
	function & appendRow($row)
	{
		if($this->_doPaging)
		{
			...
			if($page > 0 && !isset($this->_list[$page - 1]))
				die("Please, increase number of records per print page to display report properly");
			$this->_list[$page][] = $row;
	...
	function getFormattedRow($value)
	{
		...
		{		
			...
			if ($this->fieldsArr[$i]['viewFormat'] == FORMAT_DATABASE_IMAGE)
			{
				...
				$val = ProcessLargeText(GetData($value, $this->fieldsArr[$i]['name'], $this->fieldsArr[$i]['viewFormat']), "field=".rawurlencode(htmlspecialchars($this->fieldsArr[$i]['name'])).$keylink, "", $this->mode);
			}else{
