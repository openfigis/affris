...
if($inlineadd == ADD_INLINE)
	$templatefile = "vw_ingredientclass_inline_add.htm";
else
	$templatefile = "vw_ingredientclass_add.htm";
...
//	processing IngredientClassID - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		...
		if(!($value===false))
		{
				...
				$value=md5($value);
			$avalues["IngredientClassID"]=$value;
		}
		}
//	processibng IngredientClassID - end
//	processing IngredientClass - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		...
		if(!($value===false))
		{
				...
				$value=md5($value);
			$avalues["IngredientClass"]=$value;
		}
		}
//	processibng IngredientClass - end



	if(postvalue("table")=="vw_ingredientclass" && postvalue("field")=="IName") 
	{
		if(!array_key_exists("Description4",$avalues))
			$avalues["Description4"]=postvalue("category");
	}
	if(postvalue("table")=="vw_ingredientclass" && postvalue("field")=="Description3") 
	{
		if(!array_key_exists("Description2",$avalues))
			$avalues["Description2"]=postvalue("category");
	}
	if(postvalue("table")=="vw_ingredientclass" && postvalue("field")=="Description4") 
	{
		if(!array_key_exists("Description2",$avalues))
			$avalues["Description2"]=postvalue("category");
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if(GetTableData($strTableName,".edit",false) && $permis['edit'])
						$message .='&nbsp;<a href=\'vw_ingredientclass_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if(GetTableData($strTableName,".view",false) && $permis['search'])
						$message .='&nbsp;<a href=\'vw_ingredientclass_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: vw_ingredientclass_".$pageObject->getPageType().".php");
...
{
	$defvalues["Description3"]="Select";
}


if(postvalue("table")=="vw_ingredientclass" && postvalue("field")=="IName") 
	$defvalues["Description4"]=postvalue("category");
if(postvalue("table")=="vw_ingredientclass" && postvalue("field")=="Description3") 
	$defvalues["Description2"]=postvalue("category");
if(postvalue("table")=="vw_ingredientclass" && postvalue("field")=="Description4") 
	$defvalues["Description2"]=postvalue("category");
if($readavalues)
{
	...
	$defvalues["IngredientClassID"]=@$avalues["IngredientClassID"];
	$defvalues["IngredientClass"]=@$avalues["IngredientClass"];
}
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$masterquery="mastertable=vw%5Fingredientclass";
	$masterquery.="&masterkey1=".rawurlencode($data["IngredientID"]);
	$showDetailKeys["vw_fullingredientelementanalysis"]=$masterquery;
	$masterquery="mastertable=vw%5Fingredientclass";
	$masterquery.="&masterkey1=".rawurlencode($data["IngredientID"]);
	$showDetailKeys["vw_fullingredientproxanalysis"]=$masterquery;
	$masterquery="mastertable=vw%5Fingredientclass";
	$masterquery.="&masterkey1=".rawurlencode($data["IngredientID"]);
	$showDetailKeys["vw_ingredientspecassociation"]=$masterquery;
...
//	IngredientClassID - 
	...
	{	
		$value="";
			$value = ProcessLargeText(GetData($data,"IngredientClassID", ""),"field=IngredientClassID".$keylink,"",MODE_LIST);
	$showValues["IngredientClassID"] = $value;
	$showFields[] = "IngredientClassID";
		$showRawValues["IngredientClassID"] = substr($data["IngredientClassID"],0,100);
	}	
//	IngredientClass - 
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	{	
		$value="";
			$value = ProcessLargeText(GetData($data,"IngredientClass", ""),"field=IngredientClass".$keylink,"",MODE_LIST);
	$showValues["IngredientClass"] = $value;
	$showFields[] = "IngredientClass";
		$showRawValues["IngredientClass"] = substr($data["IngredientClass"],0,100);
	}	
