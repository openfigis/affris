...
//	this code must be executed after the inlineedit.js is loaded
	...
	$jscode_end.='
			window.inlineEditing'.$id.' = new InlineEditing(\'vw_feed\',\'php\','.$id.');
			'.$afteredited_handler;
...
"\nbLoading=false;\r\n";
	...
	$jscode.="MASTER_PREVIEW_TABLE='vw_feed_masterpreview.php';\r\n";
...
$body = array();
if($mode==LIST_SIMPLE)
	$html_begin.="<form name=\"frmSearch\" method=\"GET\" action=\"vw_feed_list.php\">";
else
{
	$html_begin.="<form name=\"frmSearch".$id."\" target=\"flyframe".$id."\" method=\"GET\" action=\"vw_feed_list.php\">";
...
$maxpages=1;
$rowid=0;
//orderlinkattrs for FName
$href="vw_feed_list.php?orderby=aFName";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
...
$xt->assign("FName_orderlinkattrs",$orderlinkattrs);
$xt->assign("FName_fieldheader",true);
//orderlinkattrs for Technology
$href="vw_feed_list.php?orderby=aTechnology";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
	...
	$orderlinkattrs=" href=\"".$href."\" OnMouseDown=\"sort(event,this.href);\" OnMouseOver=\"addspan(event);\" OnMouseMove=\"movespan(event);\" OnMouseOut=\"delspan();\"";
$xt->assign("Technology_orderlinkattrs",$orderlinkattrs);
$xt->assign("Technology_fieldheader",true);
//orderlinkattrs for CountryOrigin
$href="vw_feed_list.php?orderby=aCountryOrigin";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
	...
	$orderlinkattrs=" href=\"".$href."\" OnMouseDown=\"sort(event,this.href);\" OnMouseOver=\"addspan(event);\" OnMouseMove=\"movespan(event);\" OnMouseOut=\"delspan();\"";
$xt->assign("CountryOrigin_orderlinkattrs",$orderlinkattrs);
$xt->assign("CountryOrigin_fieldheader",true);
//orderlinkattrs for FeedType
$href="vw_feed_list.php?orderby=aFeedType";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
	...
	$orderlinkattrs=" href=\"".$href."\" OnMouseDown=\"sort(event,this.href);\" OnMouseOver=\"addspan(event);\" OnMouseMove=\"movespan(event);\" OnMouseOut=\"delspan();\"";
$xt->assign("FeedType_orderlinkattrs",$orderlinkattrs);
$xt->assign("FeedType_fieldheader",true);
...
//Session key fields for sort		
	...
	if($condition)
	{
		for($i=0;$i<$lenArr;$i++)
		{
			...
			{
				$href="vw_feed_list.php?orderby=a".GoodFieldName($order_field);
			}
			else
			{
				$href="vw_feed_list.php?orderby=".($order_dir=="a" ? "d" : "a").GoodFieldName($order_field);	
			}
			$orderlinkattrs="";
			if($mode==LIST_LOOKUP)
			{
...
if(no_output_done() && count($selected_recs) && $mode==LIST_SIMPLE){	
	...
	header("Location: vw_feed_".$pageName."?a=return");
	...
	if($maxpages>1)
	{
		...
		if($mode==LIST_SIMPLE)
			$jscode_end.="window.GotoPage = function(nPageNumber)
				{
					window.location='vw_feed_list.php?goto='+nPageNumber;
				};";
		else
			$jscode_end.="window.GotoPage".$id." = function (nPageNumber)
				{
					window.frames['flyframe".$id."'].location='vw_feed_list.php?".$lookupparams."&goto='+nPageNumber;
				};";
...
//	detail tables
	...
	$detailkeys=array();
	$detailkeys[]="FeedID";
	$masterquery="mastertable=vw%5Ffeed";
...
//	key fields
	...
	$record["editlink_attrs"]="href=\"vw_feed_edit.php?".$editlink."\" id=\"editlink".$recid."\"";
	$record["inlineeditlink_attrs"]= "href=\"vw_feed_edit.php?".$editlink."\" onclick=\"return inlineEditing".$id.".inlineEdit('".$recid."','".$editlink."');\" id=\"ieditlink".$recid."\"";
	$record["copylink_attrs"]="href=\"vw_feed_add.php?".$copylink."\" id=\"copylink".$recid."\"";
	$record["viewlink_attrs"]="href=\"vw_feed_view.php?".$editlink."\" id=\"viewlink".$recid."\"";
	if($mode!=LIST_LOOKUP)
	{
...
//	FName - 
			$value="";
				$value = ProcessLargeText(GetData($data,"FName", ""),"field=FName".$keylink,"",MODE_LIST);
			$record["FName_value"]=$value;

//	Technology - 
			$value="";
				$value = ProcessLargeText(GetData($data,"Technology", ""),"field=Technology".$keylink,"",MODE_LIST);
			$record["Technology_value"]=$value;

//	CountryOrigin - 
			$value="";
				$value = ProcessLargeText(GetData($data,"CountryOrigin", ""),"field=CountryOrigin".$keylink,"",MODE_LIST);
			$record["CountryOrigin_value"]=$value;

//	FeedType - 
			$value="";
				$value = ProcessLargeText(GetData($data,"FeedType", ""),"field=FeedType".$keylink,"",MODE_LIST);
			$record["FeedType_value"]=$value;
		...
		if(!strlen($record["FName_value"]))
			$record["FName_value"]="&nbsp";
		if(!strlen($record["Technology_value"]))
			$record["Technology_value"]="&nbsp";
		if(!strlen($record["CountryOrigin_value"]))
			$record["CountryOrigin_value"]="&nbsp";
		if(!strlen($record["FeedType_value"]))
			$record["FeedType_value"]="&nbsp";
...
if($mode==LIST_SIMPLE)
{
	...
	$xt->assign("printlink_attrs","href=\"vw_feed_print.php\" onclick=\"window.open('vw_feed_print.php','wPrint');return false;\"");
	$xt->assign("printalllink_attrs","href=\"vw_feed_print.php?all=1\" onclick=\"window.open('vw_feed_print.php?all=1','wPrint');return false;\"");
	$xt->assign("export_link",$allow_export);
	$xt->assign("exportlink_attrs","href=\"vw_feed_export.php\" onclick=\"window.open('vw_feed_export.php','wExport');return false;\"");
	...
	if(!\$('input[@type=checkbox][@checked][@name^=selection]').length)
		return true;
	document.forms.frmAdmin.action='vw_feed_print.php';
	document.forms.frmAdmin.target='_blank';
	document.forms.frmAdmin.submit(); 
	document.forms.frmAdmin.action='vw_feed_list.php'; 
	document.forms.frmAdmin.target='_self';return false\"
	href=\"vw_feed_print.php\"");
	$xt->assign("exportselected_link",$allow_export);
	$xt->assign("exportselectedlink_attrs","disptype=\"control1\" onclick=\"
	if(!\$('input[@type=checkbox][@checked][@name^=selection]').length)
		return true;
	document.forms.frmAdmin.action='vw_feed_export.php';
	document.forms.frmAdmin.target='_blank';
	document.forms.frmAdmin.submit(); 
	document.forms.frmAdmin.action='vw_feed_list.php'; 
	document.forms.frmAdmin.target='_self';return false;\"
	href=\"vw_feed_export.php\"");
	
	$xt->assign("add_link",$allow_add);
	$xt->assign("copy_column",$allow_add);
	$xt->assign("addlink_attrs","href=\"vw_feed_add.php\" onClick=\"window.location.href='vw_feed_add.php'\"");
	$xt->assign("inlineadd_link",$allow_add);
	$xt->assign("inlineaddlink_attrs","href=\"vw_feed_add.php\" onclick=\"return inlineEditing".$id.".inlineAdd(flyid++,null,'vw_feed_add.php');\"");
...
for (i=0;i<frmAdmin.elements['selection[]'].length;++i) 
	...
	$xt->assign("editselectedlink_attrs","href=\"vw_feed_edit.php\" disptype=\"control1\" name=\"edit_selected".$id."\" onclick=\"\$('input[@type=checkbox][@checked][@id^=check]').each(function(i){
...
//	$xt->assign("checkbox_column",true);
	$xt->assign("inlineadd_link",$allow_add);
	$xt->assign("inlineaddlink_attrs","href=\"vw_feed_add.php\" onclick=\"return inlineEditing".$id.".inlineAdd(flyid++,".$id.",'vw_feed_add.php');\"");
//	$xt->assign("inlineedit_column",$allow_edit);
}
...
$xt->assign("Technology_fieldheadercolumn",true);
$xt->assign("Technology_fieldcolumn",true);
$xt->assign("Technology_fieldfootercolumn",true);
$xt->assign("CountryOrigin_fieldheadercolumn",true);
$xt->assign("CountryOrigin_fieldcolumn",true);
$xt->assign("CountryOrigin_fieldfootercolumn",true);
$xt->assign("FeedType_fieldheadercolumn",true);
$xt->assign("FeedType_fieldcolumn",true);
$xt->assign("FeedType_fieldfootercolumn",true);
	
$display_grid = $allow_search && $rowsfound;

$xt->assign("asearch_link",$allow_search);
$xt->assign("asearchlink_attrs","href=\"vw_feed_search.php\" onclick=\"window.location.href='vw_feed_search.php';return false;\"");
$xt->assign("import_link",$allow_import);
$xt->assign("importlink_attrs","href=\"vw_feed_import.php\" onclick=\"window.location.href='vw_feed_import.php';return false;\"");
...
if($display_grid)
{
	if($mode==LIST_SIMPLE)
		...
		"<form method=\"POST\" action=\"vw_feed_list.php\" name=\"frmAdmin\" id=\"frmAdmin\">
		<input type=\"hidden\" id=\"a\" name=\"a\" value=\"delete\">",
		"</form>");
	elseif($mode==LIST_LOOKUP)
		$xt->assign_section("grid_block",
		"<form method=\"POST\" action=\"vw_feed_list.php\" name=\"frmAdmin".$id."\" id=\"frmAdmin".$id."\" target=\"flyframe".$id."\">
...
if($mode==LIST_SIMPLE)
{
	...
	$xt->assign("recordspp_attrs","onchange=\"javascript: document.location='vw_feed_list.php?pagesize='+this.options[this.selectedIndex].value;\"");
	$xt->assign("pages_block",$allow_search && $rowsfound);
}
else
	$xt->assign("recordspp_attrs","onchange=\"javascript: window.frames['flyframe".$id."'].location='vw_feed_list.php?".$lookupparams."&pagesize='+this.options[this.selectedIndex].value;\"");
...
//	include proper masterlist.php code
	...
	$params=array("detailtable"=>"vw_feed","keys"=>$masterkeys);
	$master=array();
