...
//	determine which field should be used to select values
	$lookupSelectField="";
	$lookupSelectField="isShownDetail";
...
else if(@$_REQUEST["a"]=="search")
{
	...
	$basicSearchFieldsArr[]="FName";
	$basicSearchFieldsArr[]="BrandName";
	$basicSearchFieldsArr[]="Technology";
	$basicSearchFieldsArr[]="FeedYear";
	$basicSearchFieldsArr[]="Stage";
	$basicSearchFieldsArr[]="CountryOrigin";
	$basicSearchFieldsArr[]="FisDetail";
	$basicSearchFieldsArr[]="FDataSource";
	$basicSearchFieldsArr[]="FeedType";
	$basicSearchFieldsArr[]="FeedAnalysisType";
	$basicSearchFieldsArr[]="FAValue";
	$basicSearchFieldsArr[]="isShownDetail";
...
//	this code must be executed after the inlineedit.js is loaded
	...
	$jscode_end.='
			window.inlineEditing'.$id.' = new InlineEditing(\'vw_feedanalysis\',\'php\','.$id.');
			'.$afteredited_handler;
...
"\nbLoading=false;\r\n";
	$jscode.="SUGGEST_TABLE='vw_feedanalysis_searchsuggest.php';\r\n";
	$jscode.="MASTER_PREVIEW_TABLE='vw_feedanalysis_masterpreview.php';\r\n";
...
$body = array();
if($mode==LIST_SIMPLE)
	$html_begin.="<form name=\"frmSearch\" method=\"GET\" action=\"vw_feedanalysis_list.php\">";
else
{
	$html_begin.="<form name=\"frmSearch".$id."\" target=\"flyframe".$id."\" method=\"GET\" action=\"vw_feedanalysis_list.php\">";
...
$maxpages=1;
$rowid=0;
//orderlinkattrs for FisDetail
$href="vw_feedanalysis_list.php?orderby=aFisDetail";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
...
$xt->assign("FisDetail_orderlinkattrs",$orderlinkattrs);
$xt->assign("FisDetail_fieldheader",true);
//orderlinkattrs for FeedAnalysisType
$href="vw_feedanalysis_list.php?orderby=aFeedAnalysisType";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
	...
	$orderlinkattrs=" href=\"".$href."\" OnMouseDown=\"sort(event,this.href);\" OnMouseOver=\"addspan(event);\" OnMouseMove=\"movespan(event);\" OnMouseOut=\"delspan();\"";
$xt->assign("FeedAnalysisType_orderlinkattrs",$orderlinkattrs);
$xt->assign("FeedAnalysisType_fieldheader",true);
//orderlinkattrs for FAValue
$href="vw_feedanalysis_list.php?orderby=aFAValue";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
	...
	$orderlinkattrs=" href=\"".$href."\" OnMouseDown=\"sort(event,this.href);\" OnMouseOver=\"addspan(event);\" OnMouseMove=\"movespan(event);\" OnMouseOut=\"delspan();\"";
$xt->assign("FAValue_orderlinkattrs",$orderlinkattrs);
$xt->assign("FAValue_fieldheader",true);
//orderlinkattrs for isShownDetail
$href="vw_feedanalysis_list.php?orderby=aisShownDetail";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
	...
	$orderlinkattrs=" href=\"".$href."\" OnMouseDown=\"sort(event,this.href);\" OnMouseOver=\"addspan(event);\" OnMouseMove=\"movespan(event);\" OnMouseOut=\"delspan();\"";
$xt->assign("isShownDetail_orderlinkattrs",$orderlinkattrs);
...
//Session key fields for sort		
	...
	if($condition)
	{
		for($i=0;$i<$lenArr;$i++)
		{
			...
			{
				$href="vw_feedanalysis_list.php?orderby=a".GoodFieldName($order_field);
			}
			else
			{
				$href="vw_feedanalysis_list.php?orderby=".($order_dir=="a" ? "d" : "a").GoodFieldName($order_field);	
			}
			$orderlinkattrs="";
			if($mode==LIST_LOOKUP)
			{
...
if(no_output_done() && count($selected_recs) && $mode==LIST_SIMPLE){	
	...
	header("Location: vw_feedanalysis_".$pageName."?a=return");
...
{  
	...
	{
		$strWhere = "1=0";
		if($where=StrWhereExpression("FName", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhereExpression("BrandName", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhereExpression("Technology", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhereExpression("FeedYear", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhereExpression("Stage", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhereExpression("CountryOrigin", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhereExpression("FisDetail", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhereExpression("FDataSource", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhereExpression("FeedType", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhereExpression("FeedAnalysisType", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhereExpression("FAValue", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhereExpression("isShownDetail", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
...
{
	$where ="";
		$where.= GetFullFieldName("FisDetail")."=".make_db_value("FisDetail",$_SESSION[$sessionPrefix."_masterkey1"]);
	$strWhereClause = whereAdd($strWhereClause,$where);
}
if($mastertable=="vw_feed")
{
	$where ="";
		$where.= GetFullFieldName("FeedID")."=".make_db_value("FeedID",$_SESSION[$sessionPrefix."_masterkey1"]);
	$strWhereClause = whereAdd($strWhereClause,$where);
}
	...
	if($maxpages>1)
	{
		...
		if($mode==LIST_SIMPLE)
			$jscode_end.="window.GotoPage = function(nPageNumber)
				{
					window.location='vw_feedanalysis_list.php?goto='+nPageNumber;
				};";
		else
			$jscode_end.="window.GotoPage".$id." = function (nPageNumber)
				{
					window.frames['flyframe".$id."'].location='vw_feedanalysis_list.php?".$lookupparams."&goto='+nPageNumber;
				};";
...
//	key fields
	...
	$record["editlink_attrs"]="href=\"vw_feedanalysis_edit.php?".$editlink."\" id=\"editlink".$recid."\"";
	$record["inlineeditlink_attrs"]= "href=\"vw_feedanalysis_edit.php?".$editlink."\" onclick=\"return inlineEditing".$id.".inlineEdit('".$recid."','".$editlink."');\" id=\"ieditlink".$recid."\"";
	$record["copylink_attrs"]="href=\"vw_feedanalysis_add.php?".$copylink."\" id=\"copylink".$recid."\"";
	$record["viewlink_attrs"]="href=\"vw_feedanalysis_view.php?".$editlink."\" id=\"viewlink".$recid."\"";
	if($mode!=LIST_LOOKUP)
	{
...
//	isShownDetail - 
			$value="";
				$value = ProcessLargeText(GetData($data,"isShownDetail", ""),"field=isShownDetail".$keylink,"",MODE_LIST);
			$record["isShownDetail_value"]=$value;

//	FisDetail - 
			$value="";
				$value = ProcessLargeText(GetData($data,"FisDetail", ""),"field=FisDetail".$keylink,"",MODE_LIST);
			$record["FisDetail_value"]=$value;

//	FeedAnalysisType - 
			$value="";
				$value = ProcessLargeText(GetData($data,"FeedAnalysisType", ""),"field=FeedAnalysisType".$keylink,"",MODE_LIST);
			$record["FeedAnalysisType_value"]=$value;

//	FAValue - Number
			$value="";
				$value = ProcessLargeText(GetData($data,"FAValue", "Number"),"field=FAValue".$keylink,"",MODE_LIST);
			$record["FAValue_value"]=$value;
		...
		if(!strlen($record["FisDetail_value"]))
			$record["FisDetail_value"]="&nbsp";
		if(!strlen($record["FeedAnalysisType_value"]))
			$record["FeedAnalysisType_value"]="&nbsp";
		if(!strlen($record["FAValue_value"]))
			$record["FAValue_value"]="&nbsp";
		if(!strlen($record["isShownDetail_value"]))
			$record["isShownDetail_value"]="&nbsp";
...
if($mode==LIST_SIMPLE)
{
	...
	$xt->assign("printlink_attrs","href=\"vw_feedanalysis_print.php\" onclick=\"window.open('vw_feedanalysis_print.php','wPrint');return false;\"");
	$xt->assign("printalllink_attrs","href=\"vw_feedanalysis_print.php?all=1\" onclick=\"window.open('vw_feedanalysis_print.php?all=1','wPrint');return false;\"");
	$xt->assign("export_link",$allow_export);
	$xt->assign("exportlink_attrs","href=\"vw_feedanalysis_export.php\" onclick=\"window.open('vw_feedanalysis_export.php','wExport');return false;\"");
	...
	if(!\$('input[@type=checkbox][@checked][@name^=selection]').length)
		return true;
	document.forms.frmAdmin.action='vw_feedanalysis_print.php';
	document.forms.frmAdmin.target='_blank';
	document.forms.frmAdmin.submit(); 
	document.forms.frmAdmin.action='vw_feedanalysis_list.php'; 
	document.forms.frmAdmin.target='_self';return false\"
	href=\"vw_feedanalysis_print.php\"");
	$xt->assign("exportselected_link",$allow_export);
	$xt->assign("exportselectedlink_attrs","disptype=\"control1\" onclick=\"
	if(!\$('input[@type=checkbox][@checked][@name^=selection]').length)
		return true;
	document.forms.frmAdmin.action='vw_feedanalysis_export.php';
	document.forms.frmAdmin.target='_blank';
	document.forms.frmAdmin.submit(); 
	document.forms.frmAdmin.action='vw_feedanalysis_list.php'; 
	document.forms.frmAdmin.target='_self';return false;\"
	href=\"vw_feedanalysis_export.php\"");
	
	$xt->assign("add_link",$allow_add);
	$xt->assign("copy_column",$allow_add);
	$xt->assign("addlink_attrs","href=\"vw_feedanalysis_add.php\" onClick=\"window.location.href='vw_feedanalysis_add.php'\"");
	$xt->assign("inlineadd_link",$allow_add);
	$xt->assign("inlineaddlink_attrs","href=\"vw_feedanalysis_add.php\" onclick=\"return inlineEditing".$id.".inlineAdd(flyid++,null,'vw_feedanalysis_add.php');\"");
...
for (i=0;i<frmAdmin.elements['selection[]'].length;++i) 
	...
	$xt->assign("editselectedlink_attrs","href=\"vw_feedanalysis_edit.php\" disptype=\"control1\" name=\"edit_selected".$id."\" onclick=\"\$('input[@type=checkbox][@checked][@id^=check]').each(function(i){
...
//	$xt->assign("checkbox_column",true);
	$xt->assign("inlineadd_link",$allow_add);
	$xt->assign("inlineaddlink_attrs","href=\"vw_feedanalysis_add.php\" onclick=\"return inlineEditing".$id.".inlineAdd(flyid++,".$id.",'vw_feedanalysis_add.php');\"");
//	$xt->assign("inlineedit_column",$allow_edit);
}
...
$xt->assign("FisDetail_fieldheadercolumn",true);
$xt->assign("FisDetail_fieldcolumn",true);
$xt->assign("FisDetail_fieldfootercolumn",true);
$xt->assign("FeedAnalysisType_fieldheadercolumn",true);
$xt->assign("FeedAnalysisType_fieldcolumn",true);
$xt->assign("FeedAnalysisType_fieldfootercolumn",true);
$xt->assign("FAValue_fieldheadercolumn",true);
$xt->assign("FAValue_fieldcolumn",true);
$xt->assign("FAValue_fieldfootercolumn",true);
	
$display_grid = $allow_search && $rowsfound;

$xt->assign("asearch_link",$allow_search);
$xt->assign("asearchlink_attrs","href=\"vw_feedanalysis_search.php\" onclick=\"window.location.href='vw_feedanalysis_search.php';return false;\"");
$xt->assign("import_link",$allow_import);
$xt->assign("importlink_attrs","href=\"vw_feedanalysis_import.php\" onclick=\"window.location.href='vw_feedanalysis_import.php';return false;\"");
...
if($display_grid)
{
	if($mode==LIST_SIMPLE)
		...
		"<form method=\"POST\" action=\"vw_feedanalysis_list.php\" name=\"frmAdmin\" id=\"frmAdmin\">
		<input type=\"hidden\" id=\"a\" name=\"a\" value=\"delete\">",
		"</form>");
	elseif($mode==LIST_LOOKUP)
		$xt->assign_section("grid_block",
		"<form method=\"POST\" action=\"vw_feedanalysis_list.php\" name=\"frmAdmin".$id."\" id=\"frmAdmin".$id."\" target=\"flyframe".$id."\">
...
if($mode==LIST_SIMPLE)
{
	...
	$xt->assign("recordspp_attrs","onchange=\"javascript: document.location='vw_feedanalysis_list.php?pagesize='+this.options[this.selectedIndex].value;\"");
	$xt->assign("pages_block",$allow_search && $rowsfound);
}
else
	$xt->assign("recordspp_attrs","onchange=\"javascript: window.frames['flyframe".$id."'].location='vw_feedanalysis_list.php?".$lookupparams."&pagesize='+this.options[this.selectedIndex].value;\"");
...
//	include proper masterlist.php code
	...
	$params=array("detailtable"=>"vw_feedanalysis","keys"=>$masterkeys);
	$master=array();
...
{
//	include proper masterlist.php code
	include("include/vw_feed_masterlist.php");
	$masterkeys[]=@$_SESSION[$sessionPrefix."_masterkey1"];
	$params=array("detailtable"=>"vw_feedanalysis","keys"=>$masterkeys);
	$master=array();
	$master["func"]="DisplayMasterTableInfo_vw_feed";
	$master["params"]=$params;
	$xt->assignbyref("showmasterfile",$master);
	$xt->assign("mastertable_block",true);
	$xt->assign("backtomasterlink_attrs","href=\"vw_feed_list.php?a=return\"");
}
if($mastertable=="vw_feedspec")
{
//	include proper masterlist.php code
	include("include/vw_feedspec_masterlist.php");
	$masterkeys[]=@$_SESSION[$sessionPrefix."_masterkey1"];
	$params=array("detailtable"=>"vw_feedanalysis","keys"=>$masterkeys);
	$master=array();
	$master["func"]="DisplayMasterTableInfo_vw_feedspec";
	$master["params"]=$params;
	$xt->assignbyref("showmasterfile",$master);
	$xt->assign("mastertable_block",true);
	$xt->assign("backtomasterlink_attrs","href=\"vw_feedspec_list.php?a=return\"");
}
...
$templatefile = "vw_feedanalysis_list.htm";
