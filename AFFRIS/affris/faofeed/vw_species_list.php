...
else if(@$_REQUEST["a"]=="search")
{
	...
	$basicSearchFieldsArr[]="SpecName";
	$basicSearchFieldsArr[]="CommonName";
	$basicSearchFieldsArr[]="Group";
	$basicSearchFieldsArr[]="Environment";
	$basicSearchFieldsArr[]="FeedHabit";
...
//	this code must be executed after the inlineedit.js is loaded
	...
	$jscode_end.='
			window.inlineEditing'.$id.' = new InlineEditing(\'vw_species\',\'php\','.$id.');
			'.$afteredited_handler;
...
"\nbLoading=false;\r\n";
	...
	$jscode.="MASTER_PREVIEW_TABLE='vw_species_masterpreview.php';\r\n";
...
$body = array();
if($mode==LIST_SIMPLE)
	$html_begin.="<form name=\"frmSearch\" method=\"GET\" action=\"vw_species_list.php\">";
else
{
	$html_begin.="<form name=\"frmSearch".$id."\" target=\"flyframe".$id."\" method=\"GET\" action=\"vw_species_list.php\">";
...
$maxpages=1;
$rowid=0;
//orderlinkattrs for SpecName
$href="vw_species_list.php?orderby=aSpecName";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
...
$xt->assign("SpecName_orderlinkattrs",$orderlinkattrs);
$xt->assign("SpecName_fieldheader",true);
//orderlinkattrs for CommonName
$href="vw_species_list.php?orderby=aCommonName";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
	...
	$orderlinkattrs=" href=\"".$href."\" OnMouseDown=\"sort(event,this.href);\" OnMouseOver=\"addspan(event);\" OnMouseMove=\"movespan(event);\" OnMouseOut=\"delspan();\"";
$xt->assign("CommonName_orderlinkattrs",$orderlinkattrs);
$xt->assign("CommonName_fieldheader",true);
//orderlinkattrs for Hybrid
$href="vw_species_list.php?orderby=aHybrid";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
	...
	$orderlinkattrs=" href=\"".$href."\" OnMouseDown=\"sort(event,this.href);\" OnMouseOver=\"addspan(event);\" OnMouseMove=\"movespan(event);\" OnMouseOut=\"delspan();\"";
$xt->assign("Hybrid_orderlinkattrs",$orderlinkattrs);
$xt->assign("Hybrid_fieldheader",true);
//orderlinkattrs for Variety
$href="vw_species_list.php?orderby=aVariety";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
	...
	$orderlinkattrs=" href=\"".$href."\" OnMouseDown=\"sort(event,this.href);\" OnMouseOver=\"addspan(event);\" OnMouseMove=\"movespan(event);\" OnMouseOut=\"delspan();\"";
$xt->assign("Variety_orderlinkattrs",$orderlinkattrs);
$xt->assign("Variety_fieldheader",true);
//orderlinkattrs for Family
$href="vw_species_list.php?orderby=aFamily";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
	...
	$orderlinkattrs=" href=\"".$href."\" OnMouseDown=\"sort(event,this.href);\" OnMouseOver=\"addspan(event);\" OnMouseMove=\"movespan(event);\" OnMouseOut=\"delspan();\"";
$xt->assign("Family_orderlinkattrs",$orderlinkattrs);
$xt->assign("Family_fieldheader",true);
//orderlinkattrs for Group
$href="vw_species_list.php?orderby=aGroup";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
	...
	$orderlinkattrs=" href=\"".$href."\" OnMouseDown=\"sort(event,this.href);\" OnMouseOver=\"addspan(event);\" OnMouseMove=\"movespan(event);\" OnMouseOut=\"delspan();\"";
$xt->assign("Group_orderlinkattrs",$orderlinkattrs);
$xt->assign("Group_fieldheader",true);
//orderlinkattrs for Genus
$href="vw_species_list.php?orderby=aGenus";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
	...
	$orderlinkattrs=" href=\"".$href."\" OnMouseDown=\"sort(event,this.href);\" OnMouseOver=\"addspan(event);\" OnMouseMove=\"movespan(event);\" OnMouseOut=\"delspan();\"";
$xt->assign("Genus_orderlinkattrs",$orderlinkattrs);
$xt->assign("Genus_fieldheader",true);
//orderlinkattrs for Environment
$href="vw_species_list.php?orderby=aEnvironment";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
	...
	$orderlinkattrs=" href=\"".$href."\" OnMouseDown=\"sort(event,this.href);\" OnMouseOver=\"addspan(event);\" OnMouseMove=\"movespan(event);\" OnMouseOut=\"delspan();\"";
$xt->assign("Environment_orderlinkattrs",$orderlinkattrs);
$xt->assign("Environment_fieldheader",true);
//orderlinkattrs for FeedHabit
$href="vw_species_list.php?orderby=aFeedHabit";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
	...
	$orderlinkattrs=" href=\"".$href."\" OnMouseDown=\"sort(event,this.href);\" OnMouseOver=\"addspan(event);\" OnMouseMove=\"movespan(event);\" OnMouseOut=\"delspan();\"";
$xt->assign("FeedHabit_orderlinkattrs",$orderlinkattrs);
$xt->assign("FeedHabit_fieldheader",true);
//orderlinkattrs for Country
$href="vw_species_list.php?orderby=aCountry";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
	...
	$orderlinkattrs=" href=\"".$href."\" OnMouseDown=\"sort(event,this.href);\" OnMouseOver=\"addspan(event);\" OnMouseMove=\"movespan(event);\" OnMouseOut=\"delspan();\"";
$xt->assign("Country_orderlinkattrs",$orderlinkattrs);
...
//Session key fields for sort		
	...
	if($condition)
	{
		for($i=0;$i<$lenArr;$i++)
		{
			...
			{
				$href="vw_species_list.php?orderby=a".GoodFieldName($order_field);
			}
			else
			{
				$href="vw_species_list.php?orderby=".($order_dir=="a" ? "d" : "a").GoodFieldName($order_field);	
			}
			$orderlinkattrs="";
			if($mode==LIST_LOOKUP)
			{
...
if (@$_REQUEST["mdelete"])
{
	...
	{
		$keys=array();
		$keys["SpeciesID"]=refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
		$selected_recs[]=$keys;
	}
}
elseif(@$_REQUEST["selection"])
{
	foreach(@$_REQUEST["selection"] as $keyblock)
	{
		...
		if(count($arr)<1)
			continue;
		$keys=array();
		$keys["SpeciesID"]=urldecode(@$arr[0]);
		$selected_recs[]=$keys;
	}
}
...
if(no_output_done() && count($selected_recs) && $mode==LIST_SIMPLE){	
	...
	header("Location: vw_species_".$pageName."?a=return");
...
{  
	...
	{
		$strWhere = "1=0";
		if($where=StrWhereExpression("SpecName", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhereExpression("CommonName", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhereExpression("Group", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhereExpression("Environment", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhereExpression("FeedHabit", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
	...
	if($maxpages>1)
	{
		...
		if($mode==LIST_SIMPLE)
			$jscode_end.="window.GotoPage = function(nPageNumber)
				{
					window.location='vw_species_list.php?goto='+nPageNumber;
				};";
		else
			$jscode_end.="window.GotoPage".$id." = function (nPageNumber)
				{
					window.frames['flyframe".$id."'].location='vw_species_list.php?".$lookupparams."&goto='+nPageNumber;
				};";
...
//	key fields
	...
	$copylink="";
	$keylink="";
		$keyblock.=rawurlencode($data["SpeciesID"]);
	$editlink.="editid1=".htmlspecialchars(rawurlencode($data["SpeciesID"]));
	$copylink.="copyid1=".htmlspecialchars(rawurlencode($data["SpeciesID"]));
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["SpeciesID"]));

	$record["editlink_attrs"]="href=\"vw_species_edit.php?".$editlink."\" id=\"editlink".$recid."\"";
	$record["inlineeditlink_attrs"]= "href=\"vw_species_edit.php?".$editlink."\" onclick=\"return inlineEditing".$id.".inlineEdit('".$recid."','".$editlink."');\" id=\"ieditlink".$recid."\"";
	$record["copylink_attrs"]="href=\"vw_species_add.php?".$copylink."\" id=\"copylink".$recid."\"";
	$record["viewlink_attrs"]="href=\"vw_species_view.php?".$editlink."\" id=\"viewlink".$recid."\"";
	if($mode!=LIST_LOOKUP)
	{
...
//	Country - 
		...
		if(!strlen($record["SpecName_value"]))
			$record["SpecName_value"]="&nbsp";
		if(!strlen($record["CommonName_value"]))
			$record["CommonName_value"]="&nbsp";
		if(!strlen($record["Hybrid_value"]))
			$record["Hybrid_value"]="&nbsp";
		if(!strlen($record["Variety_value"]))
			$record["Variety_value"]="&nbsp";
		if(!strlen($record["Family_value"]))
			$record["Family_value"]="&nbsp";
		if(!strlen($record["Group_value"]))
			$record["Group_value"]="&nbsp";
		if(!strlen($record["Genus_value"]))
			$record["Genus_value"]="&nbsp";
		if(!strlen($record["Environment_value"]))
			$record["Environment_value"]="&nbsp";
		if(!strlen($record["FeedHabit_value"]))
			$record["FeedHabit_value"]="&nbsp";
		if(!strlen($record["Country_value"]))
			$record["Country_value"]="&nbsp";
...
if($mode==LIST_SIMPLE)
{
	...
	$xt->assign("printlink_attrs","href=\"vw_species_print.php\" onclick=\"window.open('vw_species_print.php','wPrint');return false;\"");
	$xt->assign("printalllink_attrs","href=\"vw_species_print.php?all=1\" onclick=\"window.open('vw_species_print.php?all=1','wPrint');return false;\"");
	$xt->assign("export_link",$allow_export);
	$xt->assign("exportlink_attrs","href=\"vw_species_export.php\" onclick=\"window.open('vw_species_export.php','wExport');return false;\"");
	...
	if(!\$('input[@type=checkbox][@checked][@name^=selection]').length)
		return true;
	document.forms.frmAdmin.action='vw_species_print.php';
	document.forms.frmAdmin.target='_blank';
	document.forms.frmAdmin.submit(); 
	document.forms.frmAdmin.action='vw_species_list.php'; 
	document.forms.frmAdmin.target='_self';return false\"
	href=\"vw_species_print.php\"");
	$xt->assign("exportselected_link",$allow_export);
	$xt->assign("exportselectedlink_attrs","disptype=\"control1\" onclick=\"
	if(!\$('input[@type=checkbox][@checked][@name^=selection]').length)
		return true;
	document.forms.frmAdmin.action='vw_species_export.php';
	document.forms.frmAdmin.target='_blank';
	document.forms.frmAdmin.submit(); 
	document.forms.frmAdmin.action='vw_species_list.php'; 
	document.forms.frmAdmin.target='_self';return false;\"
	href=\"vw_species_export.php\"");
	
	$xt->assign("add_link",$allow_add);
	$xt->assign("copy_column",$allow_add);
	$xt->assign("addlink_attrs","href=\"vw_species_add.php\" onClick=\"window.location.href='vw_species_add.php'\"");
	$xt->assign("inlineadd_link",$allow_add);
	$xt->assign("inlineaddlink_attrs","href=\"vw_species_add.php\" onclick=\"return inlineEditing".$id.".inlineAdd(flyid++,null,'vw_species_add.php');\"");
...
for (i=0;i<frmAdmin.elements['selection[]'].length;++i) 
	...
	$xt->assign("editselectedlink_attrs","href=\"vw_species_edit.php\" disptype=\"control1\" name=\"edit_selected".$id."\" onclick=\"\$('input[@type=checkbox][@checked][@id^=check]').each(function(i){
...
//	$xt->assign("checkbox_column",true);
	$xt->assign("inlineadd_link",$allow_add);
	$xt->assign("inlineaddlink_attrs","href=\"vw_species_add.php\" onclick=\"return inlineEditing".$id.".inlineAdd(flyid++,".$id.",'vw_species_add.php');\"");
//	$xt->assign("inlineedit_column",$allow_edit);
}

$xt->assign("SpecName_fieldheadercolumn",true);
$xt->assign("SpecName_fieldcolumn",true);
$xt->assign("SpecName_fieldfootercolumn",true);
...
$xt->assign("Hybrid_fieldheadercolumn",true);
$xt->assign("Hybrid_fieldcolumn",true);
$xt->assign("Hybrid_fieldfootercolumn",true);
$xt->assign("Variety_fieldheadercolumn",true);
$xt->assign("Variety_fieldcolumn",true);
$xt->assign("Variety_fieldfootercolumn",true);
$xt->assign("Family_fieldheadercolumn",true);
$xt->assign("Family_fieldcolumn",true);
$xt->assign("Family_fieldfootercolumn",true);
$xt->assign("Group_fieldheadercolumn",true);
$xt->assign("Group_fieldcolumn",true);
$xt->assign("Group_fieldfootercolumn",true);
$xt->assign("Genus_fieldheadercolumn",true);
$xt->assign("Genus_fieldcolumn",true);
$xt->assign("Genus_fieldfootercolumn",true);
$xt->assign("Environment_fieldheadercolumn",true);
$xt->assign("Environment_fieldcolumn",true);
$xt->assign("Environment_fieldfootercolumn",true);
$xt->assign("FeedHabit_fieldheadercolumn",true);
$xt->assign("FeedHabit_fieldcolumn",true);
$xt->assign("FeedHabit_fieldfootercolumn",true);
...
$xt->assign("asearchlink_attrs","href=\"vw_species_search.php\" onclick=\"window.location.href='vw_species_search.php';return false;\"");
$xt->assign("import_link",$allow_import);
$xt->assign("importlink_attrs","href=\"vw_species_import.php\" onclick=\"window.location.href='vw_species_import.php';return false;\"");
...
if($display_grid)
{
	if($mode==LIST_SIMPLE)
		...
		"<form method=\"POST\" action=\"vw_species_list.php\" name=\"frmAdmin\" id=\"frmAdmin\">
		<input type=\"hidden\" id=\"a\" name=\"a\" value=\"delete\">",
		"</form>");
	elseif($mode==LIST_LOOKUP)
		$xt->assign_section("grid_block",
		"<form method=\"POST\" action=\"vw_species_list.php\" name=\"frmAdmin".$id."\" id=\"frmAdmin".$id."\" target=\"flyframe".$id."\">
...
if($mode==LIST_SIMPLE)
{
	...
	$xt->assign("recordspp_attrs","onchange=\"javascript: document.location='vw_species_list.php?pagesize='+this.options[this.selectedIndex].value;\"");
	$xt->assign("pages_block",$allow_search && $rowsfound);
}
else
	$xt->assign("recordspp_attrs","onchange=\"javascript: window.frames['flyframe".$id."'].location='vw_species_list.php?".$lookupparams."&pagesize='+this.options[this.selectedIndex].value;\"");
...
$templatefile = "vw_species_list.htm";
