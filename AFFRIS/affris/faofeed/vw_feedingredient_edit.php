...
$templatefile = "vw_feedingredient_edit.htm";
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// redirect
	header("Location: vw_feedingredient_".$pageName."?".$keyGetQ);
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: vw_feedingredient_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if(!$inlineedit)
{
	...
	$body["begin"]=$includes."
	<form name=\"editform\" id=\"editform\" encType=\"multipart/form-data\" method=\"post\" action=\"vw_feedingredient_edit.php\" ".$onsubmit.">".
...
//create a line where and order for the two queries
	...
	if(count($next))
	{
		...
		$xt->assign("nextbutton_attrs","align=\"absmiddle\" onclick=\"window.location.href='vw_feedingredient_edit.php?".$nextlink."'\"");
	...
	if(count($prev))
	{
		...
		$xt->assign("prevbutton_attrs","align=\"absmiddle\" onclick=\"window.location.href='vw_feedingredient_edit.php?".$prevlink."'\"");
...
$control_FValue["params"]["value"]=@$data["FValue"];

$control_FValue["params"]["id"]=$record_id;
if($inlineedit)
	$control_FValue["params"]["mode"]="inline_edit";
else
	$control_FValue["params"]["mode"]="edit";
