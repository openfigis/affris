...
else if(@$_REQUEST["a"]=="search")
{
	...
	$basicSearchFieldsArr[]="DigestibilityType";
	$basicSearchFieldsArr[]="Country";
...
//	this code must be executed after the inlineedit.js is loaded
	...
	$jscode_end.='
			window.inlineEditing'.$id.' = new InlineEditing(\'vw_digestibility\',\'php\','.$id.');
			'.$afteredited_handler;
...
"\nbLoading=false;\r\n";
	...
	$jscode.="MASTER_PREVIEW_TABLE='vw_digestibility_masterpreview.php';\r\n";
...
$body = array();
if($mode==LIST_SIMPLE)
	$html_begin.="<form name=\"frmSearch\" method=\"GET\" action=\"vw_digestibility_list.php\">";
else
{
	$html_begin.="<form name=\"frmSearch".$id."\" target=\"flyframe".$id."\" method=\"GET\" action=\"vw_digestibility_list.php\">";
...
$href="vw_digestibility_list.php?orderby=aIName";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
...
$href="vw_digestibility_list.php?orderby=aSpecName";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
...
//orderlinkattrs for DigestibilityType
$href="vw_digestibility_list.php?orderby=aDigestibilityType";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
	...
	$orderlinkattrs=" href=\"".$href."\" OnMouseDown=\"sort(event,this.href);\" OnMouseOver=\"addspan(event);\" OnMouseMove=\"movespan(event);\" OnMouseOut=\"delspan();\"";
$xt->assign("DigestibilityType_orderlinkattrs",$orderlinkattrs);
$xt->assign("DigestibilityType_fieldheader",true);
//orderlinkattrs for DValue
$href="vw_digestibility_list.php?orderby=aDValue";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
	...
	$orderlinkattrs=" href=\"".$href."\" OnMouseDown=\"sort(event,this.href);\" OnMouseOver=\"addspan(event);\" OnMouseMove=\"movespan(event);\" OnMouseOut=\"delspan();\"";
$xt->assign("DValue_orderlinkattrs",$orderlinkattrs);
$xt->assign("DValue_fieldheader",true);
...
//Session key fields for sort		
	...
	if($condition)
	{
		for($i=0;$i<$lenArr;$i++)
		{
			...
			{
				$href="vw_digestibility_list.php?orderby=a".GoodFieldName($order_field);
			}
			else
			{
				$href="vw_digestibility_list.php?orderby=".($order_dir=="a" ? "d" : "a").GoodFieldName($order_field);	
			}
			$orderlinkattrs="";
			if($mode==LIST_LOOKUP)
			{
...
if(no_output_done() && count($selected_recs) && $mode==LIST_SIMPLE){	
	...
	header("Location: vw_digestibility_".$pageName."?a=return");
...
{  
	...
	else
	{
		...
		if($where=StrWhereExpression("CommonName", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhereExpression("DigestibilityType", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhereExpression("Country", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
	...
	if($maxpages>1)
	{
		...
		if($mode==LIST_SIMPLE)
			$jscode_end.="window.GotoPage = function(nPageNumber)
				{
					window.location='vw_digestibility_list.php?goto='+nPageNumber;
				};";
		else
			$jscode_end.="window.GotoPage".$id." = function (nPageNumber)
				{
					window.frames['flyframe".$id."'].location='vw_digestibility_list.php?".$lookupparams."&goto='+nPageNumber;
				};";
...
//	key fields
	...
	$record["editlink_attrs"]="href=\"vw_digestibility_edit.php?".$editlink."\" id=\"editlink".$recid."\"";
	$record["inlineeditlink_attrs"]= "href=\"vw_digestibility_edit.php?".$editlink."\" onclick=\"return inlineEditing".$id.".inlineEdit('".$recid."','".$editlink."');\" id=\"ieditlink".$recid."\"";
	$record["copylink_attrs"]="href=\"vw_digestibility_add.php?".$copylink."\" id=\"copylink".$recid."\"";
	$record["viewlink_attrs"]="href=\"vw_digestibility_view.php?".$editlink."\" id=\"viewlink".$recid."\"";
	if($mode!=LIST_LOOKUP)
	{
...
//	DigestibilityType - 
			$value="";
				$value = ProcessLargeText(GetData($data,"DigestibilityType", ""),"field=DigestibilityType".$keylink,"",MODE_LIST);
			$record["DigestibilityType_value"]=$value;

//	DValue - Number
			$value="";
				$value = ProcessLargeText(GetData($data,"DValue", "Number"),"field=DValue".$keylink,"",MODE_LIST);
			$record["DValue_value"]=$value;
		...
		if(!strlen($record["DigestibilityType_value"]))
			$record["DigestibilityType_value"]="&nbsp";
		if(!strlen($record["DValue_value"]))
			$record["DValue_value"]="&nbsp";
...
if($mode==LIST_SIMPLE)
{
	...
	$xt->assign("printlink_attrs","href=\"vw_digestibility_print.php\" onclick=\"window.open('vw_digestibility_print.php','wPrint');return false;\"");
	$xt->assign("printalllink_attrs","href=\"vw_digestibility_print.php?all=1\" onclick=\"window.open('vw_digestibility_print.php?all=1','wPrint');return false;\"");
	$xt->assign("export_link",$allow_export);
	$xt->assign("exportlink_attrs","href=\"vw_digestibility_export.php\" onclick=\"window.open('vw_digestibility_export.php','wExport');return false;\"");
	...
	if(!\$('input[@type=checkbox][@checked][@name^=selection]').length)
		return true;
	document.forms.frmAdmin.action='vw_digestibility_print.php';
	document.forms.frmAdmin.target='_blank';
	document.forms.frmAdmin.submit(); 
	document.forms.frmAdmin.action='vw_digestibility_list.php'; 
	document.forms.frmAdmin.target='_self';return false\"
	href=\"vw_digestibility_print.php\"");
	$xt->assign("exportselected_link",$allow_export);
	$xt->assign("exportselectedlink_attrs","disptype=\"control1\" onclick=\"
	if(!\$('input[@type=checkbox][@checked][@name^=selection]').length)
		return true;
	document.forms.frmAdmin.action='vw_digestibility_export.php';
	document.forms.frmAdmin.target='_blank';
	document.forms.frmAdmin.submit(); 
	document.forms.frmAdmin.action='vw_digestibility_list.php'; 
	document.forms.frmAdmin.target='_self';return false;\"
	href=\"vw_digestibility_export.php\"");
	
	$xt->assign("add_link",$allow_add);
	$xt->assign("copy_column",$allow_add);
	$xt->assign("addlink_attrs","href=\"vw_digestibility_add.php\" onClick=\"window.location.href='vw_digestibility_add.php'\"");
	$xt->assign("inlineadd_link",$allow_add);
	$xt->assign("inlineaddlink_attrs","href=\"vw_digestibility_add.php\" onclick=\"return inlineEditing".$id.".inlineAdd(flyid++,null,'vw_digestibility_add.php');\"");
...
for (i=0;i<frmAdmin.elements['selection[]'].length;++i) 
	...
	$xt->assign("editselectedlink_attrs","href=\"vw_digestibility_edit.php\" disptype=\"control1\" name=\"edit_selected".$id."\" onclick=\"\$('input[@type=checkbox][@checked][@id^=check]').each(function(i){
...
//	$xt->assign("checkbox_column",true);
	$xt->assign("inlineadd_link",$allow_add);
	$xt->assign("inlineaddlink_attrs","href=\"vw_digestibility_add.php\" onclick=\"return inlineEditing".$id.".inlineAdd(flyid++,".$id.",'vw_digestibility_add.php');\"");
//	$xt->assign("inlineedit_column",$allow_edit);
}
...
$xt->assign("DigestibilityType_fieldheadercolumn",true);
$xt->assign("DigestibilityType_fieldcolumn",true);
$xt->assign("DigestibilityType_fieldfootercolumn",true);
$xt->assign("DValue_fieldheadercolumn",true);
$xt->assign("DValue_fieldcolumn",true);
$xt->assign("DValue_fieldfootercolumn",true);
	
$display_grid = $allow_search && $rowsfound;

$xt->assign("asearch_link",$allow_search);
$xt->assign("asearchlink_attrs","href=\"vw_digestibility_search.php\" onclick=\"window.location.href='vw_digestibility_search.php';return false;\"");
$xt->assign("import_link",$allow_import);
$xt->assign("importlink_attrs","href=\"vw_digestibility_import.php\" onclick=\"window.location.href='vw_digestibility_import.php';return false;\"");
...
if($display_grid)
{
	if($mode==LIST_SIMPLE)
		...
		"<form method=\"POST\" action=\"vw_digestibility_list.php\" name=\"frmAdmin\" id=\"frmAdmin\">
		<input type=\"hidden\" id=\"a\" name=\"a\" value=\"delete\">",
		"</form>");
	elseif($mode==LIST_LOOKUP)
		$xt->assign_section("grid_block",
		"<form method=\"POST\" action=\"vw_digestibility_list.php\" name=\"frmAdmin".$id."\" id=\"frmAdmin".$id."\" target=\"flyframe".$id."\">
...
if($mode==LIST_SIMPLE)
{
	...
	$xt->assign("recordspp_attrs","onchange=\"javascript: document.location='vw_digestibility_list.php?pagesize='+this.options[this.selectedIndex].value;\"");
	$xt->assign("pages_block",$allow_search && $rowsfound);
}
else
	$xt->assign("recordspp_attrs","onchange=\"javascript: window.frames['flyframe".$id."'].location='vw_digestibility_list.php?".$lookupparams."&pagesize='+this.options[this.selectedIndex].value;\"");
...
$templatefile = "vw_digestibility_list.htm";
