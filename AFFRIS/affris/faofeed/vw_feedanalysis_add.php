...
if($inlineedit==ADD_INLINE)
	$templatefile = "vw_feedanalysis_inline_add.htm";
else
	$templatefile = "vw_feedanalysis_add.htm";
...
//	processing FeedAnalysisTypeID - start
	...
	if(!($value===false))
	{
		...
		$blobfields[]="FeedAnalysisTypeID";
		$avalues["FeedAnalysisTypeID"]=$value;
	}
	}
//	processibng FeedAnalysisTypeID - end
//	processing FeedAnalysisType - start
	...
	if(!($value===false))
	{
		...
		$blobfields[]="FeedAnalysisType";
		$avalues["FeedAnalysisType"]=$value;
	}
	}
//	processibng FeedAnalysisType - end
//	processing FATTagName - start
	...
	if(!($value===false))
	{
		...
		$blobfields[]="FATTagName";
		$avalues["FATTagName"]=$value;
	}
	}
...
//	insert masterkey value if exists and if not specified
	...
	{
		$avalues["FisDetail"]=prepare_for_db("FisDetail",$_SESSION[$strTableName."_masterkey1"]);
	}
...
if (no_output_done() && $inlineedit==ADD_SIMPLE && $IsSaved){
	...
	// redirect
	header("Location: vw_feedanalysis_".$pageName);
...
if(@$_SESSION[$strTableName."_mastertable"]=="vw_feedingredient")
{
	$defvalues["FisDetail"]=@$_SESSION[$strTableName."_masterkey1"];
}
if(@$_SESSION[$strTableName."_mastertable"]=="vw_feed")
{
...
if($readavalues)
{
	...
	$defvalues["FeedAnalysisTypeID"]=@$avalues["FeedAnalysisTypeID"];
	$defvalues["FeedAnalysisType"]=@$avalues["FeedAnalysisType"];
	$defvalues["FATTagName"]=@$avalues["FATTagName"];
	...
	$defvalues["FAValue"]=@$avalues["FAValue"];
...
if($inlineedit!=ADD_INLINE)
{
	...
	{
		$body["begin"]=$includes.
		"<form name=\"editform\" id=\"editform\" encType=\"multipart/form-data\" method=\"post\" action=\"vw_feedanalysis_add.php\" ".$onsubmit.">".
	...
	else
	{
		...
		$body["begin"]="<form name=\"editform".$id."\" id=\"editform".$id."\" encType=\"multipart/form-data\" method=\"post\" action=\"vw_feedanalysis_add.php\" ".$onsubmit." target=\"flyframe".$id."\">".
		"<input type=hidden name=\"a\" value=\"added\">".
...
$control_FeedAnalysisTypeID["params"]["value"]=@$defvalues["FeedAnalysisTypeID"];
$control_FeedAnalysisTypeID["params"]["id"]=$record_id;
if($inlineedit==ADD_INLINE || $inlineedit==ADD_ONTHEFLY)
	$control_FeedAnalysisTypeID["params"]["mode"]="inline_add";
else
	$control_FeedAnalysisTypeID["params"]["mode"]="add";
...
$control_FeedAnalysisType["params"]["value"]=@$defvalues["FeedAnalysisType"];
$control_FeedAnalysisType["params"]["id"]=$record_id;
if($inlineedit==ADD_INLINE || $inlineedit==ADD_ONTHEFLY)
	$control_FeedAnalysisType["params"]["mode"]="inline_add";
else
	$control_FeedAnalysisType["params"]["mode"]="add";
...
$control_FATTagName["params"]["value"]=@$defvalues["FATTagName"];
$control_FATTagName["params"]["id"]=$record_id;
if($inlineedit==ADD_INLINE || $inlineedit==ADD_ONTHEFLY)
	$control_FATTagName["params"]["mode"]="inline_add";
else
	$control_FATTagName["params"]["mode"]="add";
...
$control_FAValue["params"]["value"]=@$defvalues["FAValue"];
$control_FAValue["params"]["id"]=$record_id;
if($inlineedit==ADD_INLINE || $inlineedit==ADD_ONTHEFLY)
	$control_FAValue["params"]["mode"]="inline_add";
else
	$control_FAValue["params"]["mode"]="add";
