...
$body=array();
$body["begin"]="<form action=\"vw_fullingredientelementanalysis_export.php\" method=get id=frmexport name=frmexport>";
$body["end"]="</form>";
$xt->assignbyref("body",$body);
$xt->display("vw_fullingredientelementanalysis_export.htm");
...
function ExportToExcel()
{
	...
	header("Content-Disposition: attachment;Filename=vw_fullingredientelementanalysis.xls");
...
function ExportToWord()
{
	...
	header("Content-Disposition: attachment;Filename=vw_fullingredientelementanalysis.doc");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=vw_fullingredientelementanalysis.xml");
	...
	{
		echo "<row>\r\n";
		$field=htmlspecialchars(XMLNameEncode("IngredientID"));
		echo "<".$field.">";
		echo htmlspecialchars(GetData($row,"IngredientID",""));
		echo "</".$field.">\r\n";
...
function ExportToCSV()
{
	...
	header("Content-Disposition: attachment;Filename=vw_fullingredientelementanalysis.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		if($outstr!="")
			$outstr.=",";
			$format="";
		$outstr.='"'.htmlspecialchars(GetData($row,"IngredientID",$format)).'"';
		if($outstr!="")
			$outstr.=",";
			$format="";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
	echo '<td style="width: 100" x:str>'.PrepareForExcel("Ingredient ID").'</td>';	
	...
	echo '<td style="width: 100" x:str>'.PrepareForExcel("IValue").'</td>';	
	}
	else
	{
...
// write data rows
	...
	echo '<td>';
		...
		$format="";
			echo htmlspecialchars(GetData($row,"IngredientID",$format));
	echo '</td>';
	echo '<td>';
