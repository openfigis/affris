...
//	this code must be executed after the inlineedit.js is loaded
	...
	$jscode_end.='
			window.inlineEditing'.$id.' = new InlineEditing(\'vw_fullingredientelementanalysis\',\'php\','.$id.');
			'.$afteredited_handler;
...
"\nbLoading=false;\r\n";
	...
	$jscode.="MASTER_PREVIEW_TABLE='vw_fullingredientelementanalysis_masterpreview.php';\r\n";
...
$body = array();
if($mode==LIST_SIMPLE)
	$html_begin.="<form name=\"frmSearch\" method=\"GET\" action=\"vw_fullingredientelementanalysis_list.php\">";
else
{
	$html_begin.="<form name=\"frmSearch".$id."\" target=\"flyframe".$id."\" method=\"GET\" action=\"vw_fullingredientelementanalysis_list.php\">";
...
$href="vw_fullingredientelementanalysis_list.php?orderby=aEName";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
...
$href="vw_fullingredientelementanalysis_list.php?orderby=aCommonName";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
...
$href="vw_fullingredientelementanalysis_list.php?orderby=aTagName";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
...
$href="vw_fullingredientelementanalysis_list.php?orderby=aDescription";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
	...
	$orderlinkattrs=" href=\"".$href."\" OnMouseDown=\"sort(event,this.href);\" OnMouseOver=\"addspan(event);\" OnMouseMove=\"movespan(event);\" OnMouseOut=\"delspan();\"";
$xt->assign("Description_orderlinkattrs",$orderlinkattrs);
$xt->assign("Description_fieldheader",true);
//orderlinkattrs for UnitSymbol
$href="vw_fullingredientelementanalysis_list.php?orderby=aUnitSymbol";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
	...
	$orderlinkattrs=" href=\"".$href."\" OnMouseDown=\"sort(event,this.href);\" OnMouseOver=\"addspan(event);\" OnMouseMove=\"movespan(event);\" OnMouseOut=\"delspan();\"";
$xt->assign("UnitSymbol_orderlinkattrs",$orderlinkattrs);
$xt->assign("UnitSymbol_fieldheader",true);
//orderlinkattrs for IValue
$href="vw_fullingredientelementanalysis_list.php?orderby=aIValue";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
...
//Session key fields for sort		
	...
	if($condition)
	{
		for($i=0;$i<$lenArr;$i++)
		{
			...
			{
				$href="vw_fullingredientelementanalysis_list.php?orderby=a".GoodFieldName($order_field);
			}
			else
			{
				$href="vw_fullingredientelementanalysis_list.php?orderby=".($order_dir=="a" ? "d" : "a").GoodFieldName($order_field);	
			}
			$orderlinkattrs="";
			if($mode==LIST_LOOKUP)
			{
...
if(no_output_done() && count($selected_recs) && $mode==LIST_SIMPLE){	
	...
	header("Location: vw_fullingredientelementanalysis_".$pageName."?a=return");
	...
	if($maxpages>1)
	{
		...
		if($mode==LIST_SIMPLE)
			$jscode_end.="window.GotoPage = function(nPageNumber)
				{
					window.location='vw_fullingredientelementanalysis_list.php?goto='+nPageNumber;
				};";
		else
			$jscode_end.="window.GotoPage".$id." = function (nPageNumber)
				{
					window.frames['flyframe".$id."'].location='vw_fullingredientelementanalysis_list.php?".$lookupparams."&goto='+nPageNumber;
				};";
...
//	key fields
	...
	$record["editlink_attrs"]="href=\"vw_fullingredientelementanalysis_edit.php?".$editlink."\" id=\"editlink".$recid."\"";
	$record["inlineeditlink_attrs"]= "href=\"vw_fullingredientelementanalysis_edit.php?".$editlink."\" onclick=\"return inlineEditing".$id.".inlineEdit('".$recid."','".$editlink."');\" id=\"ieditlink".$recid."\"";
	$record["copylink_attrs"]="href=\"vw_fullingredientelementanalysis_add.php?".$copylink."\" id=\"copylink".$recid."\"";
	$record["viewlink_attrs"]="href=\"vw_fullingredientelementanalysis_view.php?".$editlink."\" id=\"viewlink".$recid."\"";
	if($mode!=LIST_LOOKUP)
	{
...
//	UnitSymbol - 
		...
		if(!strlen($record["UnitSymbol_value"]))
			$record["UnitSymbol_value"]="&nbsp";
...
if($mode==LIST_SIMPLE)
{
	...
	$xt->assign("printlink_attrs","href=\"vw_fullingredientelementanalysis_print.php\" onclick=\"window.open('vw_fullingredientelementanalysis_print.php','wPrint');return false;\"");
	$xt->assign("printalllink_attrs","href=\"vw_fullingredientelementanalysis_print.php?all=1\" onclick=\"window.open('vw_fullingredientelementanalysis_print.php?all=1','wPrint');return false;\"");
	$xt->assign("export_link",$allow_export);
	$xt->assign("exportlink_attrs","href=\"vw_fullingredientelementanalysis_export.php\" onclick=\"window.open('vw_fullingredientelementanalysis_export.php','wExport');return false;\"");
	...
	if(!\$('input[@type=checkbox][@checked][@name^=selection]').length)
		return true;
	document.forms.frmAdmin.action='vw_fullingredientelementanalysis_print.php';
	document.forms.frmAdmin.target='_blank';
	document.forms.frmAdmin.submit(); 
	document.forms.frmAdmin.action='vw_fullingredientelementanalysis_list.php'; 
	document.forms.frmAdmin.target='_self';return false\"
	href=\"vw_fullingredientelementanalysis_print.php\"");
	$xt->assign("exportselected_link",$allow_export);
	$xt->assign("exportselectedlink_attrs","disptype=\"control1\" onclick=\"
	if(!\$('input[@type=checkbox][@checked][@name^=selection]').length)
		return true;
	document.forms.frmAdmin.action='vw_fullingredientelementanalysis_export.php';
	document.forms.frmAdmin.target='_blank';
	document.forms.frmAdmin.submit(); 
	document.forms.frmAdmin.action='vw_fullingredientelementanalysis_list.php'; 
	document.forms.frmAdmin.target='_self';return false;\"
	href=\"vw_fullingredientelementanalysis_export.php\"");
	
	$xt->assign("add_link",$allow_add);
	$xt->assign("copy_column",$allow_add);
	$xt->assign("addlink_attrs","href=\"vw_fullingredientelementanalysis_add.php\" onClick=\"window.location.href='vw_fullingredientelementanalysis_add.php'\"");
	$xt->assign("inlineadd_link",$allow_add);
	$xt->assign("inlineaddlink_attrs","href=\"vw_fullingredientelementanalysis_add.php\" onclick=\"return inlineEditing".$id.".inlineAdd(flyid++,null,'vw_fullingredientelementanalysis_add.php');\"");
...
for (i=0;i<frmAdmin.elements['selection[]'].length;++i) 
	...
	$xt->assign("editselectedlink_attrs","href=\"vw_fullingredientelementanalysis_edit.php\" disptype=\"control1\" name=\"edit_selected".$id."\" onclick=\"\$('input[@type=checkbox][@checked][@id^=check]').each(function(i){
...
//	$xt->assign("checkbox_column",true);
	$xt->assign("inlineadd_link",$allow_add);
	$xt->assign("inlineaddlink_attrs","href=\"vw_fullingredientelementanalysis_add.php\" onclick=\"return inlineEditing".$id.".inlineAdd(flyid++,".$id.",'vw_fullingredientelementanalysis_add.php');\"");
//	$xt->assign("inlineedit_column",$allow_edit);
}
...
$xt->assign("asearchlink_attrs","href=\"vw_fullingredientelementanalysis_search.php\" onclick=\"window.location.href='vw_fullingredientelementanalysis_search.php';return false;\"");
$xt->assign("import_link",$allow_import);
$xt->assign("importlink_attrs","href=\"vw_fullingredientelementanalysis_import.php\" onclick=\"window.location.href='vw_fullingredientelementanalysis_import.php';return false;\"");
...
if($display_grid)
{
	if($mode==LIST_SIMPLE)
		...
		"<form method=\"POST\" action=\"vw_fullingredientelementanalysis_list.php\" name=\"frmAdmin\" id=\"frmAdmin\">
		<input type=\"hidden\" id=\"a\" name=\"a\" value=\"delete\">",
		"</form>");
	elseif($mode==LIST_LOOKUP)
		$xt->assign_section("grid_block",
		"<form method=\"POST\" action=\"vw_fullingredientelementanalysis_list.php\" name=\"frmAdmin".$id."\" id=\"frmAdmin".$id."\" target=\"flyframe".$id."\">
...
if($mode==LIST_SIMPLE)
{
	...
	$xt->assign("recordspp_attrs","onchange=\"javascript: document.location='vw_fullingredientelementanalysis_list.php?pagesize='+this.options[this.selectedIndex].value;\"");
	$xt->assign("pages_block",$allow_search && $rowsfound);
}
else
	$xt->assign("recordspp_attrs","onchange=\"javascript: window.frames['flyframe".$id."'].location='vw_fullingredientelementanalysis_list.php?".$lookupparams."&pagesize='+this.options[this.selectedIndex].value;\"");
...
//	include proper masterlist.php code
	include("include/vw_ingredient_masterlist.php");
	...
	$xt->assign("backtomasterlink_attrs","href=\"vw_ingredient_list.php?a=return\"");
}
...
$templatefile = "vw_fullingredientelementanalysis_list.htm";
