...
include("include/vw_feedingredient_variables.php");
	...
	include("include/vw_ingredient_settings.php");
	include("include/vw_feed_settings.php");
...
else if(@$_REQUEST["a"]=="search")
{
	...
	$basicSearchFieldsArr[]="IisDetail";
	$basicSearchFieldsArr[]="FValue";
...
//	this code must be executed after the inlineedit.js is loaded
	...
	$jscode_end.='
			window.inlineEditing'.$id.' = new InlineEditing(\'vw_feedingredient\',\'php\','.$id.');
			'.$afteredited_handler;
...
"\nbLoading=false;\r\n";
	...
	$jscode.="MASTER_PREVIEW_TABLE='vw_feedingredient_masterpreview.php';\r\n";
...
$body = array();
if($mode==LIST_SIMPLE)
	$html_begin.="<form name=\"frmSearch\" method=\"GET\" action=\"vw_feedingredient_list.php\">";
else
{
	$html_begin.="<form name=\"frmSearch".$id."\" target=\"flyframe".$id."\" method=\"GET\" action=\"vw_feedingredient_list.php\">";
...
$href="vw_feedingredient_list.php?orderby=aIName";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
...
//orderlinkattrs for FValue
$href="vw_feedingredient_list.php?orderby=aFValue";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
	...
	$orderlinkattrs=" href=\"".$href."\" OnMouseDown=\"sort(event,this.href);\" OnMouseOver=\"addspan(event);\" OnMouseMove=\"movespan(event);\" OnMouseOut=\"delspan();\"";
$xt->assign("FValue_orderlinkattrs",$orderlinkattrs);
$xt->assign("FValue_fieldheader",true);
...
//Session key fields for sort		
	...
	if($condition)
	{
		for($i=0;$i<$lenArr;$i++)
		{
			...
			{
				$href="vw_feedingredient_list.php?orderby=a".GoodFieldName($order_field);
			}
			else
			{
				$href="vw_feedingredient_list.php?orderby=".($order_dir=="a" ? "d" : "a").GoodFieldName($order_field);	
			}
			$orderlinkattrs="";
			if($mode==LIST_LOOKUP)
			{
...
if(no_output_done() && count($selected_recs) && $mode==LIST_SIMPLE){	
	...
	header("Location: vw_feedingredient_".$pageName."?a=return");
...
//-----------Feed proximate analysis details special case--------------
...
//-----------Feed detailed analysis special case--------------
//"select FisDetail from vw_feedingredient where fname=$fname and fisdetail = 1"
$ta_vble1=GetFieldIndex("FeedID");
$result = mysql_query("select FisDetail from vw_feedingredient where feedid='$ta_vble1' and fisdetail = 1",$conn);
$num_rows = mysql_num_rows($result);
if($num_rows==0){
	$allow_vw_fullfeedelementanalysis=false;
}
else{
...
{  
	...
	else
	{
		...
		if($where=StrWhereExpression("IName", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhereExpression("IisDetail", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhereExpression("FValue", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
...
if($allow_search)
{
	...
	$sqlWhere="";	
	$masterkeys=array();
	$masterkeys[]="FisDetail";
	$detailkeys=array();
	$detailkeys[]="FisDetail";
	$masterwhere ="";
...
//	add a key field to the select list
	...
	foreach($masterkeys as $idx=>$val)
	{
		if($masterwhere)
			$masterwhere.=" and ";
		// may have to move outside of the foreach
			$strOriginalDetailsTable="vw_ingredient";
		$masterwhere.=AddTableWrappers("subQuery_cnt").".".AddFieldWrappers($detailkeys[$idx])."=".AddTableWrappers($strOriginalTableName).".".AddFieldWrappers($masterkeys[$idx]);
	}
		
	// select data from details table
	$dataSourceTable = "vw_ingredient";
	$detailsTableFrom = "FROM vw_ingredient ";
//	add a key field to the select list
	$subQ = "";
	foreach($detailkeys as $k)
	{
	...
	$countsql = "SELECT count(*) FROM (".$subQ.") ".AddTableWrappers("subQuery_cnt")." WHERE ".$masterwhere;
	$gsqlHead.=",(".$countsql.") as vw_ingredient_cnt ";
	...
	foreach($masterkeys as $idx=>$val)
	{
		if($masterwhere)
			$masterwhere.=" and ";
		// may have to move outside of the foreach
			$strOriginalDetailsTable="vw_feed";
		$masterwhere.=AddTableWrappers("subQuery_cnt").".".AddFieldWrappers($detailkeys[$idx])."=".AddTableWrappers($strOriginalTableName).".".AddFieldWrappers($masterkeys[$idx]);
	}
		
	// select data from details table
	$dataSourceTable = "vw_feed";
	$detailsTableFrom = "FROM vw_feed ";
//	add a key field to the select list
	$subQ = "";
	foreach($detailkeys as $k)
	{
	...
	$countsql = "SELECT count(*) FROM (".$subQ.") ".AddTableWrappers("subQuery_cnt")." WHERE ".$masterwhere;
	$gsqlHead.=",(".$countsql.") as vw_feed_cnt ";
	...
	if($maxpages>1)
	{
		...
		if($mode==LIST_SIMPLE)
			$jscode_end.="window.GotoPage = function(nPageNumber)
				{
					window.location='vw_feedingredient_list.php?goto='+nPageNumber;
				};";
		else
			$jscode_end.="window.GotoPage".$id." = function (nPageNumber)
				{
					window.frames['flyframe".$id."'].location='vw_feedingredient_list.php?".$lookupparams."&goto='+nPageNumber;
				};";
...
//	detail tables
	...
	$detailkeys=array();
	$detailkeys[]="FisDetail";
	$masterquery="mastertable=vw%5Ffeedingredient";
...
//detail tables
	...
	{			
		$d=$detailkeys[$idx];
		$masterquery.="&masterkey".($idx+1)."=".rawurlencode($data[$m]);
		$detailid[]=make_db_value($d,$data[$m],"","","vw_ingredient");
	}
	...
	$securityClause = SecuritySQL("Search", "vw_ingredient");	
	// add where 
	if (strlen($securityClause))
		$sqlWhere .= whereAdd("",$securityClause);
	...
	foreach($masterkeys as $idx=>$val)
	{
		if($masterwhere)
			$masterwhere.=" and ";
	$masterwhere.=GetFullFieldName($detailkeys[$idx],"vw_ingredient")."=".$detailid[$idx];
	}
	$data["vw_ingredient_cnt"]=gSQLRowCount_int($sqlHead,$sqlFrom,$sqlWhere,$sqlTail,$masterwhere,0);
	}

//detail tables
	$record["vw_ingredient_dtable_link"]=$allow_vw_ingredient;
	$record["vw_ingredient_dtablelink_attrs"]="href=\"vw_ingredient_list.php?".$masterquery."\" id=\"master_vw_ingredient".$recid."\"";
	$dpreview=true;
		if($data["vw_ingredient_cnt"]+0)
		$record["vw_ingredient_childcount"]=true;
	else
		$dpreview=false;
	$record["vw_ingredient_childnumber"]=$data["vw_ingredient_cnt"];
		if($dpreview)
	{
			$record["vw_ingredient_dtablelink_attrs"].=" onmouseover=\"RollDetailsLink.showPopup(this,'vw_ingredient_detailspreview.php'+this.href.substr(this.href.indexOf('?')));\" onmouseout=\"RollDetailsLink.hidePopup();\"";
		}
	...
	{			
		$d=$detailkeys[$idx];
		$masterquery.="&masterkey".($idx+1)."=".rawurlencode($data[$m]);
		$detailid[]=make_db_value($d,$data[$m],"","","vw_feed");
	}
	...
	$securityClause = SecuritySQL("Search", "vw_feed");	
	// add where 
	if (strlen($securityClause))
		$sqlWhere .= whereAdd("",$securityClause);
	...
	foreach($masterkeys as $idx=>$val)
	{
		if($masterwhere)
			$masterwhere.=" and ";
	$masterwhere.=GetFullFieldName($detailkeys[$idx],"vw_feed")."=".$detailid[$idx];
	}
	$data["vw_feed_cnt"]=gSQLRowCount_int($sqlHead,$sqlFrom,$sqlWhere,$sqlTail,$masterwhere,0);
	}

//detail tables
	$record["vw_feed_dtable_link"]=$allow_vw_feed;
	$record["vw_feed_dtablelink_attrs"]="href=\"vw_feed_list.php?".$masterquery."\" id=\"master_vw_feed".$recid."\"";
	$dpreview=true;
		if($data["vw_feed_cnt"]+0)
		$record["vw_feed_childcount"]=true;
	else
		$dpreview=false;
	$record["vw_feed_childnumber"]=$data["vw_feed_cnt"];
		if($dpreview)
	{
			$record["vw_feed_dtablelink_attrs"].=" onmouseover=\"RollDetailsLink.showPopup(this,'vw_feed_detailspreview.php'+this.href.substr(this.href.indexOf('?')));\" onmouseout=\"RollDetailsLink.hidePopup();\"";
		}
...
//	key fields
	...
	$record["editlink_attrs"]="href=\"vw_feedingredient_edit.php?".$editlink."\" id=\"editlink".$recid."\"";
	$record["inlineeditlink_attrs"]= "href=\"vw_feedingredient_edit.php?".$editlink."\" onclick=\"return inlineEditing".$id.".inlineEdit('".$recid."','".$editlink."');\" id=\"ieditlink".$recid."\"";
	$record["copylink_attrs"]="href=\"vw_feedingredient_add.php?".$copylink."\" id=\"copylink".$recid."\"";
	$record["viewlink_attrs"]="href=\"vw_feedingredient_view.php?".$editlink."\" id=\"viewlink".$recid."\"";
	if($mode!=LIST_LOOKUP)
	{
...
//	FValue - Number
			$value="";
				$value = ProcessLargeText(GetData($data,"FValue", "Number"),"field=FValue".$keylink,"",MODE_LIST);
			$record["FValue_value"]=$value;
		...
		if(!strlen($record["FValue_value"]))
			$record["FValue_value"]="&nbsp";
...
if($mode==LIST_SIMPLE)
{
	...
	$xt->assign("printlink_attrs","href=\"vw_feedingredient_print.php\" onclick=\"window.open('vw_feedingredient_print.php','wPrint');return false;\"");
	$xt->assign("printalllink_attrs","href=\"vw_feedingredient_print.php?all=1\" onclick=\"window.open('vw_feedingredient_print.php?all=1','wPrint');return false;\"");
	$xt->assign("export_link",$allow_export);
	$xt->assign("exportlink_attrs","href=\"vw_feedingredient_export.php\" onclick=\"window.open('vw_feedingredient_export.php','wExport');return false;\"");
	...
	if(!\$('input[@type=checkbox][@checked][@name^=selection]').length)
		return true;
	document.forms.frmAdmin.action='vw_feedingredient_print.php';
	document.forms.frmAdmin.target='_blank';
	document.forms.frmAdmin.submit(); 
	document.forms.frmAdmin.action='vw_feedingredient_list.php'; 
	document.forms.frmAdmin.target='_self';return false\"
	href=\"vw_feedingredient_print.php\"");
	$xt->assign("exportselected_link",$allow_export);
	$xt->assign("exportselectedlink_attrs","disptype=\"control1\" onclick=\"
	if(!\$('input[@type=checkbox][@checked][@name^=selection]').length)
		return true;
	document.forms.frmAdmin.action='vw_feedingredient_export.php';
	document.forms.frmAdmin.target='_blank';
	document.forms.frmAdmin.submit(); 
	document.forms.frmAdmin.action='vw_feedingredient_list.php'; 
	document.forms.frmAdmin.target='_self';return false;\"
	href=\"vw_feedingredient_export.php\"");
	
	$xt->assign("add_link",$allow_add);
	$xt->assign("copy_column",$allow_add);
	$xt->assign("addlink_attrs","href=\"vw_feedingredient_add.php\" onClick=\"window.location.href='vw_feedingredient_add.php'\"");
	$xt->assign("inlineadd_link",$allow_add);
	$xt->assign("inlineaddlink_attrs","href=\"vw_feedingredient_add.php\" onclick=\"return inlineEditing".$id.".inlineAdd(flyid++,null,'vw_feedingredient_add.php');\"");
...
for (i=0;i<frmAdmin.elements['selection[]'].length;++i) 
	...
	$xt->assign("editselectedlink_attrs","href=\"vw_feedingredient_edit.php\" disptype=\"control1\" name=\"edit_selected".$id."\" onclick=\"\$('input[@type=checkbox][@checked][@id^=check]').each(function(i){
	...
	if($mode!=LIST_LOOKUP)
	{
 		...
 		$xt->assign("vw_ingredient_dtable_column",$allow_vw_ingredient);
 		$xt->assign("vw_feed_dtable_column",$allow_vw_feed);
...
//	$xt->assign("checkbox_column",true);
	$xt->assign("inlineadd_link",$allow_add);
	$xt->assign("inlineaddlink_attrs","href=\"vw_feedingredient_add.php\" onclick=\"return inlineEditing".$id.".inlineAdd(flyid++,".$id.",'vw_feedingredient_add.php');\"");
//	$xt->assign("inlineedit_column",$allow_edit);
}
...
$xt->assign("FValue_fieldheadercolumn",true);
$xt->assign("FValue_fieldcolumn",true);
$xt->assign("FValue_fieldfootercolumn",true);
	
$display_grid = $allow_search && $rowsfound;

$xt->assign("asearch_link",$allow_search);
$xt->assign("asearchlink_attrs","href=\"vw_feedingredient_search.php\" onclick=\"window.location.href='vw_feedingredient_search.php';return false;\"");
$xt->assign("import_link",$allow_import);
$xt->assign("importlink_attrs","href=\"vw_feedingredient_import.php\" onclick=\"window.location.href='vw_feedingredient_import.php';return false;\"");
...
if($display_grid)
{
	if($mode==LIST_SIMPLE)
		...
		"<form method=\"POST\" action=\"vw_feedingredient_list.php\" name=\"frmAdmin\" id=\"frmAdmin\">
		<input type=\"hidden\" id=\"a\" name=\"a\" value=\"delete\">",
		"</form>");
	elseif($mode==LIST_LOOKUP)
		$xt->assign_section("grid_block",
		"<form method=\"POST\" action=\"vw_feedingredient_list.php\" name=\"frmAdmin".$id."\" id=\"frmAdmin".$id."\" target=\"flyframe".$id."\">
...
if($mode==LIST_SIMPLE)
{
	...
	$xt->assign("recordspp_attrs","onchange=\"javascript: document.location='vw_feedingredient_list.php?pagesize='+this.options[this.selectedIndex].value;\"");
	$xt->assign("pages_block",$allow_search && $rowsfound);
}
else
	$xt->assign("recordspp_attrs","onchange=\"javascript: window.frames['flyframe".$id."'].location='vw_feedingredient_list.php?".$lookupparams."&pagesize='+this.options[this.selectedIndex].value;\"");
...
//	include proper masterlist.php code
	...
	$params=array("detailtable"=>"vw_feedingredient","keys"=>$masterkeys);
	$master=array();
...
$templatefile = "vw_feedingredient_list.htm";
