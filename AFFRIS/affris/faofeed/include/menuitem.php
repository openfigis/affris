...
{	
	...
	{		
		...
		if(isset($pageName)&&$strTableName==$this->table&&$this->id!=0)
		{			
			...
			}elseif ($pageName==strtolower($this->pageType).".php"){
				$this->setAsCurrMenuElem($xt);
				return true;
			
			// if we need to set first element for this table following priority. Check as last statement
			}elseif(!$this->isSetParentElem()&&!$this->isThisPageInMenu($pageName)){				
				$this->setAsCurrMenuElem($xt);
				return true;
			}		
		}				
	...
	{	
		...
		if ($this->linkType == "Internal")
		{
			if ($this->pageType!="WebReports")
			{
				...
				$xt->assign("item".$this->id."_optionattrs",'value="'.GetTableURL($this->table).'_'.strtolower($this->pageType).'.php"');
			}
			else
			{
		...
		}elseif($this->linkType == "External")
		{
			...
			$xt->assign("item".$this->id."_optionattrs",'value="'.$this->href.'"');	
		}
	 ...
	 * @return bool
	 */
	function isThisPageInMenu($pageName) 
	{
		// get page types for this table in low case
		$pageTypesInLowCase = $this->changeKeysInLowerCaseFromArr($this->pageTypesInMenuForThisTable);
		// cut extension from $pageName
		$extLen = strlen(".php");		
