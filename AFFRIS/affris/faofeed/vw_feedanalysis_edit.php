...
$templatefile = "vw_feedanalysis_edit.htm";
...
//	processibng FeedType - end
	...
	if($condition)
	{
	$value = postvalue("value_FeedAnalysisTypeID");
	$type=postvalue("type_FeedAnalysisTypeID");
	if(FieldSubmitted("FeedAnalysisTypeID"))
	{
		$value=prepare_for_db("FeedAnalysisTypeID",$value,$type);
	}
	else
		$value=false;
	if($value!==false)
	{	
		...
		$evalues["FeedAnalysisTypeID"]=$value;
	}


//	processibng FeedAnalysisTypeID - end
	}
	$condition = !$inlineedit;

	if($condition)
	{
	$value = postvalue("value_FeedAnalysisType");
	$type=postvalue("type_FeedAnalysisType");
	if(FieldSubmitted("FeedAnalysisType"))
	{
		$value=prepare_for_db("FeedAnalysisType",$value,$type);
	}
	else
		$value=false;
	if($value!==false)
	{	
		...
		$evalues["FeedAnalysisType"]=$value;
	}


//	processibng FeedAnalysisType - end
	}
	$condition = !$inlineedit;

	if($condition)
	{
	$value = postvalue("value_FATTagName");
	$type=postvalue("type_FATTagName");
	if(FieldSubmitted("FATTagName"))
	{
		$value=prepare_for_db("FATTagName",$value,$type);
	}
	else
		$value=false;
	if($value!==false)
	{	
		...
		$evalues["FATTagName"]=$value;
	}


//	processibng FATTagName - end
	}
	$condition = !$inlineedit;
...
//	processibng UnitDecimal - end
	...
	if($condition)
	{
	$value = postvalue("value_FAValue");
	$type=postvalue("type_FAValue");
	if(FieldSubmitted("FAValue"))
	{
		$value=prepare_for_db("FAValue",$value,$type);
	}
	else
		$value=false;
	if($value!==false)
	{	
		...
		$evalues["FAValue"]=$value;
	}


//	processibng FAValue - end
	}
	$condition = !$inlineedit;
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// redirect
	header("Location: vw_feedanalysis_".$pageName."?".$keyGetQ);
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: vw_feedanalysis_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if($readevalues)
{
	...
	$data["FeedAnalysisTypeID"]=$evalues["FeedAnalysisTypeID"];
	$data["FeedAnalysisType"]=$evalues["FeedAnalysisType"];
	$data["FATTagName"]=$evalues["FATTagName"];
	...
	$data["FAValue"]=$evalues["FAValue"];
	$data["isShownDetail"]=$evalues["isShownDetail"];
}
...
//	validate field - FeedTypeID
	...
	$arrValidate[] = array(0 => "FeedTypeID", 1 => $validatetype, 2 => $second_validatetype, 3 => $strRTE,
						   4 => $lookup, 5 => $regex, 6 => $regexmessage, 7 => $regextype);
//	validate field - FeedAnalysisTypeID
	$validatetype="IsNumeric";
	$second_validatetype="IsRequired";
	$strRTE='';
	$lookup='';	
	$arrValidate[] = array(0 => "FeedAnalysisTypeID", 1 => $validatetype, 2 => $second_validatetype, 3 => $strRTE,
						   4 => $lookup, 5 => $regex, 6 => $regexmessage, 7 => $regextype);
...
//	validate field - UnitDecimal
	...
	$arrValidate[] = array(0 => "UnitDecimal", 1 => $validatetype, 2 => $second_validatetype, 3 => $strRTE,
						   4 => $lookup, 5 => $regex, 6 => $regexmessage, 7 => $regextype);
//	validate field - FAValue
	$validatetype="IsNumeric";
	$second_validatetype="";
	$strRTE='';
	$lookup='';	
	$arrValidate[] = array(0 => "FAValue", 1 => $validatetype, 2 => $second_validatetype, 3 => $strRTE,
						   4 => $lookup, 5 => $regex, 6 => $regexmessage, 7 => $regextype);
...
if(!$inlineedit)
{
	...
	$xt->assign("FeedAnalysisTypeID_fieldblock",true);
	$xt->assign("FeedAnalysisType_fieldblock",true);
	$xt->assign("FATTagName_fieldblock",true);
	...
	$body["begin"]=$includes."
	<form name=\"editform\" id=\"editform\" encType=\"multipart/form-data\" method=\"post\" action=\"vw_feedanalysis_edit.php\" ".$onsubmit.">".
...
//create a line where and order for the two queries
	...
	if(count($next))
	{
		...
		$xt->assign("nextbutton_attrs","align=\"absmiddle\" onclick=\"window.location.href='vw_feedanalysis_edit.php?".$nextlink."'\"");
	...
	if(count($prev))
	{
		...
		$xt->assign("prevbutton_attrs","align=\"absmiddle\" onclick=\"window.location.href='vw_feedanalysis_edit.php?".$prevlink."'\"");
...
//0
$control_FeedAnalysisTypeID=array();
$control_FeedAnalysisTypeID["func"]="xt_buildeditcontrol";
$control_FeedAnalysisTypeID["params"] = array();
$control_FeedAnalysisTypeID["params"]["field"]="FeedAnalysisTypeID";
$control_FeedAnalysisTypeID["params"]["value"]=@$data["FeedAnalysisTypeID"];

$control_FeedAnalysisTypeID["params"]["id"]=$record_id;
if($inlineedit)
	$control_FeedAnalysisTypeID["params"]["mode"]="inline_edit";
else
	$control_FeedAnalysisTypeID["params"]["mode"]="edit";
$xt->assignbyref("FeedAnalysisTypeID_editcontrol",$control_FeedAnalysisTypeID);
...
$control_FeedAnalysisType["params"]["value"]=@$data["FeedAnalysisType"];

$control_FeedAnalysisType["params"]["id"]=$record_id;
if($inlineedit)
	$control_FeedAnalysisType["params"]["mode"]="inline_edit";
else
	$control_FeedAnalysisType["params"]["mode"]="edit";
$xt->assignbyref("FeedAnalysisType_editcontrol",$control_FeedAnalysisType);

//0
$control_FATTagName=array();
$control_FATTagName["func"]="xt_buildeditcontrol";
$control_FATTagName["params"] = array();
$control_FATTagName["params"]["field"]="FATTagName";
$control_FATTagName["params"]["value"]=@$data["FATTagName"];

$control_FATTagName["params"]["id"]=$record_id;
if($inlineedit)
	$control_FATTagName["params"]["mode"]="inline_edit";
else
	$control_FATTagName["params"]["mode"]="edit";
$xt->assignbyref("FATTagName_editcontrol",$control_FATTagName);
...
$control_UnitID["params"]["value"]=@$data["UnitID"];

$control_UnitID["params"]["id"]=$record_id;
if($inlineedit)
	$control_UnitID["params"]["mode"]="inline_edit";
else
	$control_UnitID["params"]["mode"]="edit";
...
$control_UnitName["params"]["value"]=@$data["UnitName"];

$control_UnitName["params"]["id"]=$record_id;
if($inlineedit)
	$control_UnitName["params"]["mode"]="inline_edit";
else
	$control_UnitName["params"]["mode"]="edit";
...
$control_UnitSymbol["params"]["value"]=@$data["UnitSymbol"];

$control_UnitSymbol["params"]["id"]=$record_id;
if($inlineedit)
	$control_UnitSymbol["params"]["mode"]="inline_edit";
else
	$control_UnitSymbol["params"]["mode"]="edit";
...
$control_UnitDecimal["params"]["value"]=@$data["UnitDecimal"];

$control_UnitDecimal["params"]["id"]=$record_id;
if($inlineedit)
	$control_UnitDecimal["params"]["mode"]="inline_edit";
else
	$control_UnitDecimal["params"]["mode"]="edit";
...
$control_FAValue["params"]["value"]=@$data["FAValue"];

$control_FAValue["params"]["id"]=$record_id;
if($inlineedit)
	$control_FAValue["params"]["mode"]="inline_edit";
else
	$control_FAValue["params"]["mode"]="edit";
...
$control_isShownDetail["params"]["value"]=@$data["isShownDetail"];

$control_isShownDetail["params"]["id"]=$record_id;
if($inlineedit)
	$control_isShownDetail["params"]["mode"]="inline_edit";
else
	$control_isShownDetail["params"]["mode"]="edit";
