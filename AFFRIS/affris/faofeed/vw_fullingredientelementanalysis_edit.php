...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// redirect
	header("Location: vw_fullingredientelementanalysis_".$pageName."?".$keyGetQ);
...
if(!$inlineedit)
{
	...
	$body["begin"]=$includes."
	<form name=\"editform\" id=\"editform\" encType=\"multipart/form-data\" method=\"post\" action=\"vw_fullingredientelementanalysis_edit.php\" ".$onsubmit.">".
...
//create a line where and order for the two queries
	...
	if(count($next))
	{
		...
		$xt->assign("nextbutton_attrs","align=\"absmiddle\" onclick=\"window.location.href='vw_fullingredientelementanalysis_edit.php?".$nextlink."'\"");
	...
	if(count($prev))
	{
		...
		$xt->assign("prevbutton_attrs","align=\"absmiddle\" onclick=\"window.location.href='vw_fullingredientelementanalysis_edit.php?".$prevlink."'\"");
