...
//orderlinkattrs for Country_Origin
$href="vw_feedspec_list.php?orderby=aCountry_Origin";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
	...
	$orderlinkattrs=" href=\"".$href."\" OnMouseDown=\"sort(event,this.href);\" OnMouseOver=\"addspan(event);\" OnMouseMove=\"movespan(event);\" OnMouseOut=\"delspan();\"";
$xt->assign("Country_Origin_orderlinkattrs",$orderlinkattrs);
$xt->assign("Country_Origin_fieldheader",true);
//orderlinkattrs for Data_Source
$href="vw_feedspec_list.php?orderby=aData_Source";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
	...
	$orderlinkattrs=" href=\"".$href."\" OnMouseDown=\"sort(event,this.href);\" OnMouseOver=\"addspan(event);\" OnMouseMove=\"movespan(event);\" OnMouseOut=\"delspan();\"";
$xt->assign("Data_Source_orderlinkattrs",$orderlinkattrs);
$xt->assign("Data_Source_fieldheader",true);
//orderlinkattrs for Type
$href="vw_feedspec_list.php?orderby=aType";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
...
{  
	...
	else
	{
		...
		if($where=StrWhereExpression("Feed", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhereExpression("Details", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
...
//	Country Origin - 
			$value="";
				$value = ProcessLargeText(GetData($data,"Country Origin", ""),"field=Country+Origin".$keylink,"",MODE_LIST);
			$record["Country_Origin_value"]=$value;

//	Data Source - 
			$value="";
				$value = ProcessLargeText(GetData($data,"Data Source", ""),"field=Data+Source".$keylink,"",MODE_LIST);
			$record["Data_Source_value"]=$value;
...
//	Feed - 
		...
		if(!strlen($record["Country_Origin_value"]))
			$record["Country_Origin_value"]="&nbsp";
		if(!strlen($record["Data_Source_value"]))
			$record["Data_Source_value"]="&nbsp";
		if(!strlen($record["Type_value"]))
			$record["Type_value"]="&nbsp";
...
$xt->assign("Country_Origin_fieldheadercolumn",true);
$xt->assign("Country_Origin_fieldcolumn",true);
$xt->assign("Country_Origin_fieldfootercolumn",true);
$xt->assign("Data_Source_fieldheadercolumn",true);
$xt->assign("Data_Source_fieldcolumn",true);
$xt->assign("Data_Source_fieldfootercolumn",true);
