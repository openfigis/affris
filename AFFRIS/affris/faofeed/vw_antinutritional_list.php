...
else if(@$_REQUEST["a"]=="search")
{
	...
	$basicSearchFieldsArr[]="Treatment";
	$basicSearchFieldsArr[]="DataSource";
	$basicSearchFieldsArr[]="PartUsed";
...
//	this code must be executed after the inlineedit.js is loaded
	...
	$jscode_end.='
			window.inlineEditing'.$id.' = new InlineEditing(\'vw_antinutritional\',\'php\','.$id.');
			'.$afteredited_handler;
...
"\nbLoading=false;\r\n";
	...
	$jscode.="MASTER_PREVIEW_TABLE='vw_antinutritional_masterpreview.php';\r\n";
...
$body = array();
if($mode==LIST_SIMPLE)
	$html_begin.="<form name=\"frmSearch\" method=\"GET\" action=\"vw_antinutritional_list.php\">";
else
{
	$html_begin.="<form name=\"frmSearch".$id."\" target=\"flyframe".$id."\" method=\"GET\" action=\"vw_antinutritional_list.php\">";
...
$href="vw_antinutritional_list.php?orderby=aIName";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
...
//orderlinkattrs for AntiFactor
$href="vw_antinutritional_list.php?orderby=aAntiFactor";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
	...
	$orderlinkattrs=" href=\"".$href."\" OnMouseDown=\"sort(event,this.href);\" OnMouseOver=\"addspan(event);\" OnMouseMove=\"movespan(event);\" OnMouseOut=\"delspan();\"";
$xt->assign("AntiFactor_orderlinkattrs",$orderlinkattrs);
$xt->assign("AntiFactor_fieldheader",true);
//orderlinkattrs for ToxicLevel
$href="vw_antinutritional_list.php?orderby=aToxicLevel";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
	...
	$orderlinkattrs=" href=\"".$href."\" OnMouseDown=\"sort(event,this.href);\" OnMouseOver=\"addspan(event);\" OnMouseMove=\"movespan(event);\" OnMouseOut=\"delspan();\"";
$xt->assign("ToxicLevel_orderlinkattrs",$orderlinkattrs);
$xt->assign("ToxicLevel_fieldheader",true);
//orderlinkattrs for Treatment
$href="vw_antinutritional_list.php?orderby=aTreatment";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
	...
	$orderlinkattrs=" href=\"".$href."\" OnMouseDown=\"sort(event,this.href);\" OnMouseOver=\"addspan(event);\" OnMouseMove=\"movespan(event);\" OnMouseOut=\"delspan();\"";
$xt->assign("Treatment_orderlinkattrs",$orderlinkattrs);
$xt->assign("Treatment_fieldheader",true);
//orderlinkattrs for DataSource
$href="vw_antinutritional_list.php?orderby=aDataSource";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
	...
	$orderlinkattrs=" href=\"".$href."\" OnMouseDown=\"sort(event,this.href);\" OnMouseOver=\"addspan(event);\" OnMouseMove=\"movespan(event);\" OnMouseOut=\"delspan();\"";
$xt->assign("DataSource_orderlinkattrs",$orderlinkattrs);
$xt->assign("DataSource_fieldheader",true);
//orderlinkattrs for PartUsed
$href="vw_antinutritional_list.php?orderby=aPartUsed";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
	...
	$orderlinkattrs=" href=\"".$href."\" OnMouseDown=\"sort(event,this.href);\" OnMouseOver=\"addspan(event);\" OnMouseMove=\"movespan(event);\" OnMouseOut=\"delspan();\"";
$xt->assign("PartUsed_orderlinkattrs",$orderlinkattrs);
$xt->assign("PartUsed_fieldheader",true);
...
//Session key fields for sort		
	...
	if($condition)
	{
		for($i=0;$i<$lenArr;$i++)
		{
			...
			{
				$href="vw_antinutritional_list.php?orderby=a".GoodFieldName($order_field);
			}
			else
			{
				$href="vw_antinutritional_list.php?orderby=".($order_dir=="a" ? "d" : "a").GoodFieldName($order_field);	
			}
			$orderlinkattrs="";
			if($mode==LIST_LOOKUP)
			{
...
if(no_output_done() && count($selected_recs) && $mode==LIST_SIMPLE){	
	...
	header("Location: vw_antinutritional_".$pageName."?a=return");
...
{  
	...
	else
	{
		...
		if($where=StrWhereExpression("AntiFactor", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhereExpression("ToxicLevel", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhereExpression("Treatment", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhereExpression("DataSource", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhereExpression("PartUsed", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
	...
	if($maxpages>1)
	{
		...
		if($mode==LIST_SIMPLE)
			$jscode_end.="window.GotoPage = function(nPageNumber)
				{
					window.location='vw_antinutritional_list.php?goto='+nPageNumber;
				};";
		else
			$jscode_end.="window.GotoPage".$id." = function (nPageNumber)
				{
					window.frames['flyframe".$id."'].location='vw_antinutritional_list.php?".$lookupparams."&goto='+nPageNumber;
				};";
...
//	key fields
	...
	$record["editlink_attrs"]="href=\"vw_antinutritional_edit.php?".$editlink."\" id=\"editlink".$recid."\"";
	$record["inlineeditlink_attrs"]= "href=\"vw_antinutritional_edit.php?".$editlink."\" onclick=\"return inlineEditing".$id.".inlineEdit('".$recid."','".$editlink."');\" id=\"ieditlink".$recid."\"";
	$record["copylink_attrs"]="href=\"vw_antinutritional_add.php?".$copylink."\" id=\"copylink".$recid."\"";
	$record["viewlink_attrs"]="href=\"vw_antinutritional_view.php?".$editlink."\" id=\"viewlink".$recid."\"";
	if($mode!=LIST_LOOKUP)
	{
...
//	AntiFactor - 
			$value="";
				$value = ProcessLargeText(GetData($data,"AntiFactor", ""),"field=AntiFactor".$keylink,"",MODE_LIST);
			$record["AntiFactor_value"]=$value;

//	ToxicLevel - 
			$value="";
				$value = ProcessLargeText(GetData($data,"ToxicLevel", ""),"field=ToxicLevel".$keylink,"",MODE_LIST);
			$record["ToxicLevel_value"]=$value;

//	PartUsed - 
			$value="";
				$value = ProcessLargeText(GetData($data,"PartUsed", ""),"field=PartUsed".$keylink,"",MODE_LIST);
			$record["PartUsed_value"]=$value;

//	Treatment - 
			$value="";
				$value = ProcessLargeText(GetData($data,"Treatment", ""),"field=Treatment".$keylink,"",MODE_LIST);
			$record["Treatment_value"]=$value;

//	DataSource - 
			$value="";
				$value = ProcessLargeText(GetData($data,"DataSource", ""),"field=DataSource".$keylink,"",MODE_LIST);
			$record["DataSource_value"]=$value;
		...
		if(!strlen($record["AntiFactor_value"]))
			$record["AntiFactor_value"]="&nbsp";
		if(!strlen($record["ToxicLevel_value"]))
			$record["ToxicLevel_value"]="&nbsp";
		if(!strlen($record["Treatment_value"]))
			$record["Treatment_value"]="&nbsp";
		if(!strlen($record["DataSource_value"]))
			$record["DataSource_value"]="&nbsp";
		if(!strlen($record["PartUsed_value"]))
			$record["PartUsed_value"]="&nbsp";
...
if($mode==LIST_SIMPLE)
{
	...
	$xt->assign("printlink_attrs","href=\"vw_antinutritional_print.php\" onclick=\"window.open('vw_antinutritional_print.php','wPrint');return false;\"");
	$xt->assign("printalllink_attrs","href=\"vw_antinutritional_print.php?all=1\" onclick=\"window.open('vw_antinutritional_print.php?all=1','wPrint');return false;\"");
	$xt->assign("export_link",$allow_export);
	$xt->assign("exportlink_attrs","href=\"vw_antinutritional_export.php\" onclick=\"window.open('vw_antinutritional_export.php','wExport');return false;\"");
	...
	if(!\$('input[@type=checkbox][@checked][@name^=selection]').length)
		return true;
	document.forms.frmAdmin.action='vw_antinutritional_print.php';
	document.forms.frmAdmin.target='_blank';
	document.forms.frmAdmin.submit(); 
	document.forms.frmAdmin.action='vw_antinutritional_list.php'; 
	document.forms.frmAdmin.target='_self';return false\"
	href=\"vw_antinutritional_print.php\"");
	$xt->assign("exportselected_link",$allow_export);
	$xt->assign("exportselectedlink_attrs","disptype=\"control1\" onclick=\"
	if(!\$('input[@type=checkbox][@checked][@name^=selection]').length)
		return true;
	document.forms.frmAdmin.action='vw_antinutritional_export.php';
	document.forms.frmAdmin.target='_blank';
	document.forms.frmAdmin.submit(); 
	document.forms.frmAdmin.action='vw_antinutritional_list.php'; 
	document.forms.frmAdmin.target='_self';return false;\"
	href=\"vw_antinutritional_export.php\"");
	
	$xt->assign("add_link",$allow_add);
	$xt->assign("copy_column",$allow_add);
	$xt->assign("addlink_attrs","href=\"vw_antinutritional_add.php\" onClick=\"window.location.href='vw_antinutritional_add.php'\"");
	$xt->assign("inlineadd_link",$allow_add);
	$xt->assign("inlineaddlink_attrs","href=\"vw_antinutritional_add.php\" onclick=\"return inlineEditing".$id.".inlineAdd(flyid++,null,'vw_antinutritional_add.php');\"");
...
for (i=0;i<frmAdmin.elements['selection[]'].length;++i) 
	...
	$xt->assign("editselectedlink_attrs","href=\"vw_antinutritional_edit.php\" disptype=\"control1\" name=\"edit_selected".$id."\" onclick=\"\$('input[@type=checkbox][@checked][@id^=check]').each(function(i){
...
//	$xt->assign("checkbox_column",true);
	$xt->assign("inlineadd_link",$allow_add);
	$xt->assign("inlineaddlink_attrs","href=\"vw_antinutritional_add.php\" onclick=\"return inlineEditing".$id.".inlineAdd(flyid++,".$id.",'vw_antinutritional_add.php');\"");
//	$xt->assign("inlineedit_column",$allow_edit);
}
...
$xt->assign("asearchlink_attrs","href=\"vw_antinutritional_search.php\" onclick=\"window.location.href='vw_antinutritional_search.php';return false;\"");
$xt->assign("import_link",$allow_import);
$xt->assign("importlink_attrs","href=\"vw_antinutritional_import.php\" onclick=\"window.location.href='vw_antinutritional_import.php';return false;\"");
...
if($display_grid)
{
	if($mode==LIST_SIMPLE)
		...
		"<form method=\"POST\" action=\"vw_antinutritional_list.php\" name=\"frmAdmin\" id=\"frmAdmin\">
		<input type=\"hidden\" id=\"a\" name=\"a\" value=\"delete\">",
		"</form>");
	elseif($mode==LIST_LOOKUP)
		$xt->assign_section("grid_block",
		"<form method=\"POST\" action=\"vw_antinutritional_list.php\" name=\"frmAdmin".$id."\" id=\"frmAdmin".$id."\" target=\"flyframe".$id."\">
...
if($mode==LIST_SIMPLE)
{
	...
	$xt->assign("recordspp_attrs","onchange=\"javascript: document.location='vw_antinutritional_list.php?pagesize='+this.options[this.selectedIndex].value;\"");
	$xt->assign("pages_block",$allow_search && $rowsfound);
}
else
	$xt->assign("recordspp_attrs","onchange=\"javascript: window.frames['flyframe".$id."'].location='vw_antinutritional_list.php?".$lookupparams."&pagesize='+this.options[this.selectedIndex].value;\"");
...
$templatefile = "vw_antinutritional_list.htm";
