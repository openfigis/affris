...
$templatefile = "vw_fullfeedproxanalysis_edit.htm";
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// redirect
	header("Location: vw_fullfeedproxanalysis_".$pageName."?".$keyGetQ);
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: vw_fullfeedproxanalysis_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if($readevalues)
{
	...
	$data["ETTagName"]=$evalues["ETTagName"];
...
if(!$inlineedit)
{
	...
	$body["begin"]=$includes."
	<form name=\"editform\" id=\"editform\" encType=\"multipart/form-data\" method=\"post\" action=\"vw_fullfeedproxanalysis_edit.php\" ".$onsubmit.">".
...
//create a line where and order for the two queries
	...
	if(count($next))
	{
		...
		$xt->assign("nextbutton_attrs","align=\"absmiddle\" onclick=\"window.location.href='vw_fullfeedproxanalysis_edit.php?".$nextlink."'\"");
	...
	if(count($prev))
	{
		...
		$xt->assign("prevbutton_attrs","align=\"absmiddle\" onclick=\"window.location.href='vw_fullfeedproxanalysis_edit.php?".$prevlink."'\"");
...
$control_ElementTypeID["params"]["value"]=@$data["ElementTypeID"];

$control_ElementTypeID["params"]["id"]=$record_id;
if($inlineedit)
	$control_ElementTypeID["params"]["mode"]="inline_edit";
else
	$control_ElementTypeID["params"]["mode"]="edit";
...
$control_ETTagName["params"]["value"]=@$data["ETTagName"];

$control_ETTagName["params"]["id"]=$record_id;
if($inlineedit)
	$control_ETTagName["params"]["mode"]="inline_edit";
else
	$control_ETTagName["params"]["mode"]="edit";
...
$control_ETValue["params"]["value"]=@$data["ETValue"];

$control_ETValue["params"]["id"]=$record_id;
if($inlineedit)
	$control_ETValue["params"]["mode"]="inline_edit";
else
	$control_ETValue["params"]["mode"]="edit";
